...
static bool LanHost_Is_Local_Client_Address(LPSOCKADDR_IN address)
{
	if (address == NULL) {
		return false;
	}

	const unsigned long ip = ntohl(address->sin_addr.s_addr);
	return ip == 0 || (ip >> 24) == 127;
}

static bool Should_Use_In_Process_Send(bool is_server, LPSOCKADDR_IN p_address)
{
	if (cSinglePlayerData::Is_Single_Player()) {
		return true;
	}

	if (!cSinglePlayerData::Is_Lan_Host_Local_Client()) {
		return false;
	}
	...
	return LanHost_Is_Local_Client_Address(p_address);
}

static void Set_In_Process_Peer_Address(LPSOCKADDR_IN address, USHORT port)
{
	if (address == NULL) {
		return;
	}
...
char * Addr_As_String(sockaddr_in *addr)
{
	...
	const unsigned long a = ntohl(addr->sin_addr.s_addr);
...
void cConnection::Init_As_Client(LPSOCKADDR_IN p_server_address, unsigned short my_port)
{
   ...
   LocalId = ID_UNKNOWN;
	...
	//
   if (!cSinglePlayerData::Uses_In_Process_Client()) {
      bool is_bound;
   ...
   if (cSinglePlayerData::Uses_In_Process_Client()) {
		...
		Set_In_Process_Peer_Address(&local_server_address, 0);
		if (p_server_address != NULL && p_server_address->sin_port != 0) {
			local_server_address.sin_port = p_server_address->sin_port;
		}
		PRHost[0]->Set_Address(local_server_address);
   } else {
...
void cConnection::Init_As_Client(ULONG server_ip, USHORT server_port, unsigned short my_port)
{
   ...
   if (cSinglePlayerData::Uses_In_Process_Client()) {
		Set_In_Process_Peer_Address(&server_address, server_port);
   } else {
...
bool cConnection::Sender_Id_Tests(cPacket & packet)
{
   ...
   if (!cSinglePlayerData::Is_Single_Player() &&
       !(cSinglePlayerData::Is_Lan_Host_Local_Client() && IsServer) &&
...
int cConnection::Single_Player_recvfrom(char * data)
{
   ...
   WWASSERT(cSinglePlayerData::Uses_In_Process_Client());
...
bool cConnection::Receive_Packet()
{
   ...
   if (ret_code == 0) {
		...
		if (cSinglePlayerData::Uses_In_Process_Client() && ret_code == SOCKET_ERROR) {
			return(false);
		}
...
void cConnection::Process_Connection_Request(cPacket & packet)
{
   ...
   WWASSERT(IsServer);
	...
	if (cSinglePlayerData::Is_Lan_Host_Local_Client() &&
		 LanHost_Is_Local_Client_Address(p_address)) {
		client_address = *p_address;
		Set_In_Process_Peer_Address(&client_address, ntohs(p_address->sin_port));
		p_address = &client_address;
	}
...
memcpy(last_packet, packet.Get_Data(), last_packet_len);
   ...
   if (Should_Use_In_Process_Send(IsServer, p_address)) {
...
int cConnection::Low_Level_Receive_Wrapper(cPacket & packet)
{
   ...
   if (cSinglePlayerData::Is_Single_Player()) {
	...
	} else if (cSinglePlayerData::Is_Lan_Host_Local_Client() && !IsServer) {
		ret_code = Single_Player_recvfrom(packet.Get_Data());
	} else {

		if (cSinglePlayerData::Is_Lan_Host_Local_Client() && IsServer) {
			ret_code = Single_Player_recvfrom(packet.Get_Data());
			if (ret_code != SOCKET_ERROR && ret_code > 0) {
				Set_In_Process_Peer_Address(
					&packet.Get_From_Address_Wrapper()->FromAddress, 0);
				return ret_code;
			}
		}
	      ...
	      for (SLNode<cPacket> * objnode = p_rhost->Get_Packet_List(RELIABLE_SEND_LIST).Head();
            ...
            WWASSERT(p_packet != NULL);
				...
				if (p_packet->Get_Resend_Count() > 1) {
					resent_packets++;
					if (!cSinglePlayerData::Is_Lan_Host_Local_Client() &&
						 ThisFrameTimeMs - p_packet->Get_First_Send_Time() > 5000) {
						any_bad = true;
					}
				}
...
bool cConnection::Is_Packet_Too_Old(const cPacket *packet, cRemoteHost *rhost)
{
	...
	//
	// LAN listen-host uses in-process client/server (like skirmish). Do not
	// apply UDP connection-loss timeouts to the local client.
	//
	if (cSinglePlayerData::Is_Lan_Host_Local_Client()) {
		return(false);
	}
