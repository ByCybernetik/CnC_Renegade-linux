...
void cNetUtil::Create_Unbound_Socket(SOCKET & sock)
{
   ...
   //
   // Enable broadcasts and allow multiple LAN browsers on one machine.
   //
   int optval = TRUE;
   WSA_CHECK(setsockopt(sock, SOL_SOCKET, SO_BROADCAST, (char *) &optval, sizeof(optval)));
   WSA_CHECK(setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (char *) &optval, sizeof(optval)));
...
static ULONG Subnet_Broadcast_Address(ULONG ip_network_order)
{
	const ULONG ip = ntohl(ip_network_order);
	const ULONG a = (ip >> 24) & 0xff;
	const ULONG b = (ip >> 16) & 0xff;
	ULONG bcast;

	if (a == 10) {
		bcast = (ip & 0xFF000000u) | 0x00FFFFFFu;
	} else if (a == 192 && b == 168) {
		bcast = (ip & 0xFFFF0000u) | 0x0000FFFFu;
	} else if (a == 172 && b >= 16 && b <= 31) {
		bcast = (ip & 0xFFF00000u) | 0x000FFFFFu;
	} else if (a == 169 && b == 254) {
		bcast = (ip & 0xFFFF0000u) | 0x0000FFFFu;
	} else {
		bcast = (ip & 0xFFFFFF00u) | 0x000000FFu;
	}
...
{
   const int data_len = packet.Get_Compressed_Size_Bytes();
   const char * data = packet.Get_Data();

   SOCKADDR_IN broadcast_address;
   Create_Broadcast_Address(&broadcast_address, port);
   int bytes_sent = sendto(sock, data, data_len,
      0, (LPSOCKADDR) &broadcast_address, sizeof(SOCKADDR_IN));
#if defined(RENEGADE_LINUX)
   SOCKADDR_IN local_addresses[16];
   const int num_addresses = Get_Local_Tcpip_Addresses(local_addresses, 16);
   for (int i = 0; i < num_addresses; i++) {
		const ULONG ip = local_addresses[i].sin_addr.s_addr;
		const ULONG a = (ntohl(ip) >> 24) & 0xff;
		if (a == 127 || a >= 224) {
			continue;
		}

		SOCKADDR_IN subnet_address;
		ZeroMemory(&subnet_address, sizeof(subnet_address));
		subnet_address.sin_family = AF_INET;
		subnet_address.sin_addr.s_addr = Subnet_Broadcast_Address(ip);
		subnet_address.sin_port = htons(port);
		...
		sendto(
			...
			data_len,
			0,
			(LPSOCKADDR) &subnet_address,
			sizeof(subnet_address));
   }
#else
   (void)bytes_sent;
#endif
