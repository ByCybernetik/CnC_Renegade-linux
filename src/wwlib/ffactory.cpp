...
FileClass * SimpleFileFactoryClass::Get_File( char const *filename )
{
	...
	//
	StringClass new_name(stripped_name, false);
	if (Is_Full_Path ( new_name ) == false) {
		...
		if (!SubDirectory.Is_Empty()) {
			...
			//
			StringClass subdir(SubDirectory, false);
			...
			if (strchr(subdir,';'))
			{
				...
				while((tok=strtok(tokstart, ";")) != NULL) {
					...
					file->Set_Name( new_name.Peek_Buffer() );	// Call Set_Name to force an allocated name
					if (file->Open()) {
						file->Close();
						break;
					}
