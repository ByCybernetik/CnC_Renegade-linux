...
static bool RawFile_Match_Ci_In_Dir(const char *dir, const char *component, StringClass &matched)
{
	DIR *dh;
	struct dirent *entry;

	if (dir == NULL || component == NULL || component[0] == '\0') {
		return false;
	}
	...
	for (entry = readdir(dh); entry != NULL; entry = readdir(dh)) {
		if (entry->d_name[0] == '.') {
			continue;
		}
...
static bool RawFile_Resolve_Case_Insensitive(StringClass &filename)
{
	char norm[1024];
	char component[256];
	StringClass accum(true);
	StringClass matched(true);
	const char *path;
	const char *p;
	size_t n;

	path = filename.Peek_Buffer();
	if (path == NULL || path[0] == '\0') {
		return false;
	}

	n = 0;
	for (size_t i = 0; path[i] != '\0' && n + 1 < sizeof(norm); ++i) {
		norm[n++] = (path[i] == '\\') ? '/' : path[i];
	}
	norm[n] = '\0';
	...
	while (*p != '\0') {
		...
		while (*p != '\0' && *p != '/' && c + 1 < sizeof(component)) {
			component[c++] = *p++;
		}
		component[c] = '\0';
		if (c == 0) {
		...
		if ( strcmp( component, ".." ) == 0 ) {
			if ( strcmp( accum, "." ) != 0 ) {
				const char *last = strrchr( accum.Peek_Buffer(), '/' );
				if ( last != NULL ) {
					if ( last == accum.Peek_Buffer() ) {
						accum = ".";
					} else {
						StringClass parent( accum, true );
						parent.Erase( (int)( last - accum.Peek_Buffer() ), parent.Get_Length() );
						accum = parent;
					}
				}
			}
		...
		curdir = (strcmp(accum, ".") == 0) ? "." : accum.Peek_Buffer();
		if (!RawFile_Match_Ci_In_Dir(curdir, component, matched)) {
			return false;
		}
		...
		if (strcmp(accum, ".") == 0) {
			...
			accum.Format("%s/%s", accum.Peek_Buffer(), matched.Peek_Buffer());
		}
	}
...
static bool RawFile_Ensure_Parent_Directory(const char *path)
{
	const char *slash;
	char dir[1024];
	size_t len;
	...
	if (!RawFile_Ensure_Parent_Directory(dir)) {
		return false;
	}
...
static HANDLE_TYPE RawFile_Fopen_Read(StringClass &filename)
{
	HANDLE_TYPE handle = fopen(filename.Peek_Buffer(), "rb");
	if (handle != NULL_HANDLE) {
		return handle;
	}

	StringClass resolved(filename, true);
	if (RawFile_Resolve_Case_Insensitive(resolved)) {
		handle = fopen(resolved.Peek_Buffer(), "rb");
		if (handle != NULL_HANDLE) {
			filename = resolved;
		}
	}
...
static HANDLE_TYPE RawFile_Fopen_Write(StringClass &filename)
{
	RawFile_Ensure_Parent_Directory(filename.Peek_Buffer());
	HANDLE_TYPE handle = fopen(filename.Peek_Buffer(), "w");
	if (handle != NULL_HANDLE) {
		return handle;
	}

	{
		StringClass resolved(filename, true);
		if (RawFile_Resolve_Case_Insensitive(resolved)) {
			RawFile_Ensure_Parent_Directory(resolved.Peek_Buffer());
			handle = fopen(resolved.Peek_Buffer(), "w");
			if (handle != NULL_HANDLE) {
				filename = resolved;
			}
		}
		...
		switch (rights) {
			...
			*/
			...
			case READ:
				#ifdef _UNIX
#if defined(RENEGADE_LINUX)
					Handle = RawFile_Fopen_Read(Filename);
#else
			...
			case WRITE:
				#ifdef _UNIX
#if defined(RENEGADE_LINUX)
					Handle = RawFile_Fopen_Write(Filename);
#else
