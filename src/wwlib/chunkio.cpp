...
bool ChunkSaveClass::Begin_Micro_Chunk(uint32 id)
{
	...
	{
		const uint8 wire[MICRO_CHUNK_HEADER_WIRE_SIZE] = {
			MCHeader.Get_Type(),
			MCHeader.Get_Size()
		};
		if (Write(wire, MICRO_CHUNK_HEADER_WIRE_SIZE) != MICRO_CHUNK_HEADER_WIRE_SIZE) {
			return false;
		}
	}
#else
...
bool ChunkSaveClass::End_Micro_Chunk(void)
{
...
#if defined(RENEGADE_LINUX)
	{
		...
		if (File->Write(wire, MICRO_CHUNK_HEADER_WIRE_SIZE) != (int)MICRO_CHUNK_HEADER_WIRE_SIZE) {
			return false;
		}
	}
#else
...
#if defined(RENEGADE_LINUX)
	{
		uint8 wire[MICRO_CHUNK_HEADER_WIRE_SIZE];
		if (Read(wire, MICRO_CHUNK_HEADER_WIRE_SIZE) != MICRO_CHUNK_HEADER_WIRE_SIZE) {
			return false;
		}
		MCHeader.Set_Type(wire[0]);
		MCHeader.Set_Size(wire[1]);
	}
#else
