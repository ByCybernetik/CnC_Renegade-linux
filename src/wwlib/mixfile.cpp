...
/*
** On-disk MIX1 layout uses 32-bit little-endian fields. Win32 long is 32-bit; Linux LP64
** long is 64-bit and must not be used for header/index I/O.
*/
typedef struct
{
	char	signature[4];
...
//	WWASSERT( file );
	...
	if ( file && file->Is_Available() ) {
		...
		if ( IsValid ) {
			...
			mix_header_offset = (int)header.header_offset;
...
#if defined(RENEGADE_LINUX)
		FileClass * mix_container = Factory->Get_File( MixFilename );
		if ( mix_container ) {
			/*
			** Slurp the mix subfile into RAM. Biased RawFileClass/BufferedFileClass I/O
			** breaks byte-at-a-time INI parsing on LP64 (Seek/EOF never align with bias).
			*/
			const char *mix_path = mix_container->File_Name();
			int subsize = (int)info->Size;
			int bias_start = BaseOffset + (int)info->Offset;
			RAMFileClass *mem = new RAMFileClass( NULL, subsize );
			int bytes_read = 0;
			if ( mem != NULL && subsize > 0 ) {
				RawFileClass src( mix_path );
				src.Bias( bias_start, subsize );
				if ( src.Open( RawFileClass::READ ) ) {
					mem->Open( RAMFileClass::WRITE );
					char chunk[8192];
					while ( bytes_read < subsize ) {
						int want = subsize - bytes_read;
						if ( want > (int)sizeof( chunk ) ) {
							want = (int)sizeof( chunk );
						}
						int n = src.Read( chunk, want );
						if ( n <= 0 ) {
							break;
						}
						mem->Write( chunk, n );
			...
			Factory->Return_File( mix_container );
			file = ( bytes_read > 0 ) ? mem : NULL;
		}
#else
		file = Factory->Get_File( MixFilename );
		if ( file ) {
			((RawFileClass *)file)->Bias( BaseOffset + info->Offset, info->Size );
		}
#endif
//		WWDEBUG_SAY(( "MixFileFactoryClass::Get_File( %s ) FOUND\n", filename ));
	} else {
...
MixFileFactoryClass::Flush_Changes (void)
{
	...
	if (Get_Temp_Filename (path, full_path)) {
		...
		//
		for (int pending_index = 0; pending_index < PendingAddFileList.Count (); pending_index ++) {
			int index = pending_index;
