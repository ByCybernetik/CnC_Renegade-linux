...
#endif
{
	...
	const size_t buf_count = sizeof(temp_buffer) / sizeof(temp_buffer[0]);
	...
	retval = _vsnwprintf(temp_buffer, (int)buf_count, format, args_copy);
	va_end(args_copy);

	if (retval < 0 || (size_t)retval >= buf_count) {
		temp_buffer[buf_count - 1] = 0;
	}
...
{
	if (text != NULL && (unsigned long)(uintptr_t)text > 1UL) {
