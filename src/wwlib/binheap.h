...
class BinaryHeapClass
{
	public:
		...
		bool Has_Valid_Array (void) const
		{
			...
			const uintptr_t arr = (uintptr_t)Elements;
#if UINTPTR_MAX > 0xFFFFFFFFUL
			if (arr < 0x10000UL) {
				return false;
			}
#if defined(__linux__) || defined(__APPLE__)
			if (arr >= 0x0000800000000000UL) {
				return false;
			}
#elif defined(_WIN32) || defined(_WIN64)
			if (arr >= 0x00007FFFFFFFFFFFUL) {
				return false;
			}
#endif
#else
			if (arr < 0x00100000UL || arr >= 0x7F000000UL) {
				return false;
			}
#endif
			if (Max_Number_Of_Elements > 100000) {
				return false;
			}
			return true;
		}

		void Abandon_Corrupted_Array (void)
		{
			Elements = NULL;
			Number_Of_Elements = 0;
