...
//
//////////////////////////////////////////////////////////////////////
/* Reject NULL/low integer "pointers" (e.g. 0x1) before wcslen — Wine/MinGW crash. */
inline static bool WideString_Ptr_OK(const WCHAR *s)
{
	return (s != NULL) && ((unsigned long)(uintptr_t)s > 1UL);
}
...
public:
	...
	WideStringClass (unsigned short *string,			bool hint_temporary = false);
	WideStringClass (const unsigned short *string,	bool hint_temporary = false);
#endif
	...
	/* va_list must be passed by value on GCC/x86_64 (not by reference). */
...
WideStringClass::WideStringClass (const WCHAR *string, bool hint_temporary)
	:	m_Buffer (m_EmptyString)
{
	int len = WideString_Ptr_OK(string) ? wcslen(string) : 0;
...
WideStringClass::WideStringClass (const char *string, bool hint_temporary)
	:	m_Buffer (m_EmptyString)
{
	int len = (string != NULL && (unsigned long)(uintptr_t)string > 1UL)
		? (int)strlen(string) : 0;
	if (hint_temporary || len > 0) {
		Get_String(len + 1, hint_temporary);
	}
...
#if defined(__MINGW32__) || defined(__MINGW64__)
///////////////////////////////////////////////////////////////////
//	WideStringClass (unsigned short*)
///////////////////////////////////////////////////////////////////
inline
WideStringClass::WideStringClass (unsigned short *string, bool hint_temporary)
	:	m_Buffer (m_EmptyString)
{
	const WCHAR *wstr = reinterpret_cast<const WCHAR*>(string);
	int len = WideString_Ptr_OK(wstr) ? wcslen(wstr) : 0;
	if (hint_temporary || len > 0) {
		Get_String(len + 1, hint_temporary);
	}
	(*this) = wstr;
...
inline
WideStringClass::WideStringClass (const unsigned short *string, bool hint_temporary)
	:	m_Buffer (m_EmptyString)
{
	const WCHAR *wstr = reinterpret_cast<const WCHAR*>(string);
	int len = WideString_Ptr_OK(wstr) ? wcslen(wstr) : 0;
...
{
	if (m_Buffer == NULL || (unsigned long)(uintptr_t)m_Buffer <= 1UL) {
