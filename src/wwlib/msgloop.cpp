...
Win32_Key_Notify_Callback Win32_Key_Notify_Callback_Ptr = NULL;
...
void Windows_Message_Handler(void)
{
	...
	*/
	while (PeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE)) {
		...
		if ( Win32_Key_Notify_Callback_Ptr != NULL ) {
			switch ( msg.message ) {
				case WM_KEYDOWN:
				case WM_KEYUP:
				case WM_SYSKEYDOWN:
				case WM_SYSKEYUP:
					Win32_Key_Notify_Callback_Ptr( msg.message, msg.wParam, msg.lParam );
					break;
				default:
					break;
			}
		...
		*/
		...
		if (!_Accelerators.Has_Valid_Storage()) {
			_Accelerators.Abandon_Corrupted_Storage();
		} else
#endif
		for (int aindex = 0; aindex < _Accelerators.Count(); aindex++) {
			const AcceleratorTracker &acc = _Accelerators[aindex];
			if (acc.Window) {
				if (TranslateAccelerator(acc.Window, acc.Accelerator, &msg)) {
					processed = true;
				}
			}
			break;
		...
		*/
#if defined(__GNUC__) && defined(_WIN32)
		if (!_ModelessDialogs.Has_Valid_Storage()) {
			_ModelessDialogs.Abandon_Corrupted_Storage();
		} else
#endif
		for (int index = 0; index < _ModelessDialogs.Count(); index++) {
			const HWND dialog = _ModelessDialogs[index];
