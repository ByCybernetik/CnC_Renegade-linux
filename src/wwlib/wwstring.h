...
/* Reject NULL/low integer "pointers" (e.g. 0x1) before strlen — Wine/MinGW crash. */
inline static bool String_Ptr_OK(const TCHAR *s)
{
	return (s != NULL) && ((unsigned long)(uintptr_t)s > 1UL);
}

inline static bool String_Ptr_OK_W(const WCHAR *s)
{
	return (s != NULL) && ((unsigned long)(uintptr_t)s > 1UL);
}
...
StringClass::StringClass (const TCHAR *string, bool hint_temporary)
	:	m_Buffer (m_EmptyString)
{
	int len = String_Ptr_OK(string) ? _tcsclen(string) : 0;
...
StringClass::StringClass (const WCHAR *string, bool hint_temporary)
	:	m_Buffer (m_EmptyString)
{
	int len = String_Ptr_OK_W(string) ? wcslen(string) : 0;
	if (hint_temporary || len > 0) {
		Get_String (len + 1, hint_temporary);
	}
