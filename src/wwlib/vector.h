...
class DynamicVectorClass : public VectorClass<T>
{
	public:
		...
		/*
		** Detect ActiveCount/Vector mismatch (seen on Wine after bad persist remap).
		*/
		bool Has_Valid_Storage(void) const
		{
			...
			const uintptr_t vec = (uintptr_t)this->Vector;
#if UINTPTR_MAX > 0xFFFFFFFFUL
			if (vec < 0x10000UL) {
				return false;
			}
#if defined(__linux__) || defined(__APPLE__)
			if (vec >= 0x0000800000000000UL) {
				return false;
			}
#elif defined(_WIN32) || defined(_WIN64)
			if (vec >= 0x00007FFFFFFFFFFFUL) {
				return false;
			}
#endif
#else
			/*
			** Reject kernel-ish / sentinel values (e.g. 0x9, 0xC000C) that pass a
			** naive >64K check but are not heap blocks (Wine pathfind pool corruption).
			*/
			if (vec < 0x00100000UL || vec >= 0x7F000000UL) {
				return false;
			}
#endif
			if (this->VectorMax < ActiveCount || this->VectorMax > 100000) {
				return false;
			}
			return true;
		}
		...
		/*
		** Drop corrupted storage without calling delete[] on a bogus Vector pointer.
		*/
		void Abandon_Corrupted_Storage(void)
		{
			ActiveCount = 0;
			this->Vector = 0;
			this->VectorMax = 0;
...
bool DynamicVectorClass<T>::Add(T const & object)
{
	if (ActiveCount >= this->Length()) {
		if ((this->IsAllocated || !this->VectorMax) && GrowthStep > 0) {
			if (!VectorClass<T>::Resize(this->Length() + GrowthStep)) {
...
void DynamicVectorClass<T>::Delete_All(void) 
{
	...
	ActiveCount = 0;
	VectorClass<T>::Clear();		// Forces destructor call on each object.
	VectorClass<T>::Resize(len);
}
