...
/* On-disk micro-chunk header is exactly 2 bytes (type + size). */
...
static const unsigned MICRO_CHUNK_HEADER_WIRE_SIZE = 2;
...
/*
** Win32 level/save files store pointers as 32-bit values in micro-chunks.
** On LP64 hosts READ_MICRO_CHUNK would over-read and corrupt the stream.
*/
#if defined(RENEGADE_LINUX)
#define READ_MICRO_CHUNK_WIRE_POINTER(cload,id,var)						\
	case (id): {														\
		uint32 _wire_ptr_u32 = 0;										\
		cload.Read(&_wire_ptr_u32, sizeof(_wire_ptr_u32));				\
		*(void **)&(var) = (void *)(uintptr_t)_wire_ptr_u32;			\
		break;															\
	}
#else
#define READ_MICRO_CHUNK_WIRE_POINTER(cload,id,var) \
	READ_MICRO_CHUNK(cload, id, var)
#endif
