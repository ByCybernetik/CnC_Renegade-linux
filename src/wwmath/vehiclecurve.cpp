...
VehicleCurveClass::Evaluate (float time, Vector3 *set_val)
{
	...
	//
	// Tracked vehicles use turn_radius 0; arc lengths collapse to zero and the
	// ratios below become NaN, snapping the sampler to bogus XY (often 0,0).
	//
	if ( total_length < 0.001F ) {
		set_val->X = Keys[index0].Point.X + ( Keys[index1].Point.X - Keys[index0].Point.X ) * seg_time;
		set_val->Y = Keys[index0].Point.Y + ( Keys[index1].Point.Y - Keys[index0].Point.Y ) * seg_time;
		set_val->Z = Keys[index0].Point.Z + ( Keys[index1].Point.Z - Keys[index0].Point.Z ) * seg_time;
		m_LastTime = Keys[index0].Time + ( Keys[index1].Time - Keys[index0].Time ) * seg_time;
		return;
	}
