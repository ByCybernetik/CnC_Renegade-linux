...
/*
** Static table list. On Linux use heap storage so the list is not torn down during
** static destruction (after other subsystems may have corrupted list nodes).
*/
#if defined(RENEGADE_LINUX)
static RefMultiListClass<LookupTableClass> &LookupTables(void)
{
	static RefMultiListClass<LookupTableClass> *tables =
		new RefMultiListClass<LookupTableClass>();
	return *tables;
}
#define TABLES LookupTables()
#else
static RefMultiListClass<LookupTableClass>	_LookupTables;
#define TABLES _LookupTables
#endif
