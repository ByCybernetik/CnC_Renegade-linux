...
void DlgWOLSettings::InitConnectionSpeedCombo(void)
	...
	if (connectCombo)
		...
		if (BandwidthCheckerClass::Get_Reported_Upstream_Bandwidth())
			{
			{
				const WCHAR *bw = (const WCHAR *)BandwidthCheckerClass::Get_Bandwidth_As_String();
				if (bw == NULL || (unsigned long)(uintptr_t)bw <= 1UL) {
					bw = L"";
				}
				auto_set_str.Format(TRANSLATE (IDS_MENU_AUTO_FORMAT), bw);
			}
			}
		else
			{
