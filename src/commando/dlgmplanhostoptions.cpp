		...
		case IDC_MENU_MP_LAN_START_BUTTON:
			{
				...
				if (tab_ctrl != NULL) {
					...
					//
					bool apply_ok = tab_ctrl->Apply_Changes_On_Tabs ();
					if (apply_ok) {
...
//		Enable_Dlg_Item(IDC_NUM_PLAYERS_EDIT, false);
		...
		int max_players = The_Game ()->Get_Max_Players ();
		if (max_players <= 0) {
			max_players = 16;
	...
	if (nic_combobox != NULL) {
		...
		if (current_index < 0 && nic_count > 0) {
			nic_combobox->Set_Curr_Sel(0);
			The_Game()->Set_Ip_Address(nics[0]);
			cUserOptions::PreferredLanNic.Set(nics[0]);
		}
...
MPLanHostBasicOptionsTabClass::On_Apply (void)
{
	...
	if (player_count <= 0) {
		player_count = The_Game ()->Get_Max_Players ();
		if (player_count <= 0) {
			player_count = 16;
		}
	...
	The_Game ()->Set_Max_Players (min(player_count, NetworkObjectClass::MAX_CLIENT_COUNT - 1));
	...
	if (!GameModeManager::Find("WOL")->Is_Active() && !cGameSpyAdmin::Get_Is_Server_Gamespy_Listed()) {
		...
		if (nic_combobox != NULL) {
			...
			} else if (cNicEnum::Get_Num_Nics() > 0) {
				ULONG * nics = cNicEnum::Get_Nics();
				WWASSERT(nics != NULL);
				cUserOptions::PreferredLanNic.Set(nics[0]);
				The_Game()->Set_Ip_Address(nics[0]);
...
MPLanHostMapCycleOptionsTabClass::On_Init_Dialog (void)
{
	...
	//
	WWASSERT(PTheGameData != NULL);
	int init_time_limit = The_Game ()->Get_Time_Limit_Minutes ();
	if (init_time_limit <= 0) {
		init_time_limit = 25;
		The_Game ()->Set_Time_Limit_Minutes (init_time_limit);
	}
	((EditCtrlClass *)Get_Dlg_Item (IDC_MAP_TIME_LIMIT_EDIT))->Set_Text_Limit (3);
	Set_Dlg_Item_Int (IDC_MAP_TIME_LIMIT_EDIT, init_time_limit);
...
MPLanHostMapCycleOptionsTabClass::On_Apply (void)
{
	...
	//
	WWASSERT(PTheGameData != NULL);
	int time_limit = Get_Dlg_Item_Int (IDC_MAP_TIME_LIMIT_EDIT);
	if (time_limit <= 0) {
		time_limit = The_Game ()->Get_Time_Limit_Minutes ();
		if (time_limit <= 0) {
			time_limit = 25;
		}
		Set_Dlg_Item_Int (IDC_MAP_TIME_LIMIT_EDIT, time_limit);
	}
	The_Game ()->Set_Time_Limit_Minutes (time_limit);
	The_Game ()->Set_Do_Maps_Loop (Is_Dlg_Button_Checked (IDC_LOOP_MAPS_CHECK));

	return ChildDialogClass::On_Apply ();
}
...
MPLanHostMapCycleOptionsTabClass::Build_Map_List (void)
{
	...
	if (The_Game()->Is_Cnc()) {
		...
		file_filter.Format("%smp_*.mix", DATA_SUBDIRECTORY);
	}

#ifdef WWDEBUG
	if (cDevOptions::FilterLevelFiles.Is_False()) {
		file_filter.Format("%s*.mix", DATA_SUBDIRECTORY);
	}
#endif // WWDEBUG
