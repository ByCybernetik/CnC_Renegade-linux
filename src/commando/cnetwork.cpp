...
void cNetwork::Cleanup_Client(void)
{
   ...
   if (I_Am_Client()) {
      ...
      if (PClientConnection != NULL && PClientConnection->Is_Established()) { // i.e we did have a connection
...
void cNetwork::Init_Server(void)
{
   ...
   PServerConnection->Enable_Flow_Control(is_flow_control_enabled);
	...
	ULONG server_bind_addr = ::ntohl(The_Game()->Get_Ip_Address());
	if (GameModeManager::Find("LAN")->Is_Active() &&
		 !The_Game()->IsDedicated.Get() &&
		 !cGameSpyAdmin::Is_Gamespy_Game()) {
		// Listen-host: accept connections on all interfaces (loopback + LAN).
		server_bind_addr = 0;
	}
	...
	if (PServerConnection->Get_Local_Port() != 0 &&
		 PServerConnection->Get_Local_Port() != (USHORT)The_Game()->Get_Port()) {
		The_Game()->Set_Port(PServerConnection->Get_Local_Port());
	}
...
void cNetwork::Update(void)
{
	...
	if (I_Am_Server()) {
		if (I_Am_Client() && PClientConnection != NULL) {
...
//		}
	...
	} else if (I_Am_Client() && PClientConnection != NULL) {
...
void cNetwork::Client_Send_Packet(cPacket & packet, int mode)
{
   ...
   WWASSERT(I_Am_Client());
	...
	if (PClientConnection != NULL && PClientConnection->Is_Established()) {
...
REFUSAL_CODE cNetwork::Application_Acceptance_Handler(cPacket & packet)
{
   ...
   WWDEBUG_SAY(("cNetwork::Application_Acceptance_Handler\n"));
	...
	const int server_exe_key = cNetwork::Get_Exe_Key();
	...
	// Make sure the exe versions match (not for local single-player loopback)
	if (client_exe_key != server_exe_key) {
		if (!cSinglePlayerData::Uses_In_Process_Client()) {
			return REFUSAL_VERSION_MISMATCH;
		}
   }
