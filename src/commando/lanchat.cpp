...
void cLanChat::Process_Position_Broadcast(cPacket & packet)
{
   ...
   if (is_hosting) {
		...
		//
		// Advertise the address the broadcast came from so clients on the LAN can connect.
		//
		if (packet.Get_From_Address_Wrapper() != NULL) {
			p_game_data->Set_Ip_Address(
				packet.Get_From_Address_Wrapper()->FromAddress.sin_addr.s_addr);
		}
