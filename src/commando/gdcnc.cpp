...
bool cGameDataCnc::Is_Gameplay_Permitted(void)
{
	...
	{
		const bool lan_inprocess_host =
			GameModeManager::Find("LAN") != NULL &&
			GameModeManager::Find("LAN")->Is_Active() &&
			cSinglePlayerData::Is_Lan_Host_Local_Client() &&
			!cGameSpyAdmin::Is_Gamespy_Game() &&
			IsDedicated.Is_False();

		//
		// LAN listen-host on Linux uses in-process client/server (like skirmish).
		// Allow the host to play while alone; opposing base still exists on the map.
		//
		if (!lan_inprocess_host) {
			permitted =
