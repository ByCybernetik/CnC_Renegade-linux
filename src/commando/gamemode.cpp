...
void	GameModeManager::Think( void )
{
	...
	{
		GameModeClass *movie_mode = Find("Movie");
		const bool movie_mode_active = movie_mode != NULL && movie_mode->Is_Active();
		if (!movie_mode_active && BINKMovie::Is_Playing()) {
			BINKMovie::Update();
		}
	}
}
...
void	GameModeManager::Render( void )
{
	...
	if (!ConsoleBox.Is_Exclusive()) {
		...
		*/
		...
		// Render Bink movies after UI overlays so video is not covered.
