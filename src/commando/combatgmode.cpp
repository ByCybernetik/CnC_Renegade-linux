...
#define	TOTAL	(18.0f)
	...
	void Render(bool update_network = false)
	{
		...
		frame_dt = TimeManager::Get_Frame_Seconds();
		if (frame_dt <= 0.0f) {
			frame_dt = TimeManager::Get_Frame_Real_Seconds();
		}
		LoadTime += frame_dt;
		...
		LoadPercentage += LoadPercentageRate * frame_dt;
...
static void Pump_Loading_Screen(LoadingScreenClass &loading_screen, bool update_network)
{
	loading_screen.Render(update_network);
...
*/
	...
	if (cSinglePlayerData::Is_Lan_Host_Local_Client()) {
		if (cNetwork::PClientConnection != NULL) {
			cNetwork::PClientConnection->Clear_Resend_Counts();
		}
		if (cNetwork::PServerConnection != NULL) {
			cNetwork::PServerConnection->Clear_Resend_Counts();
		}
	}
