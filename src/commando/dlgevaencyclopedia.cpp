...
EVAEncyclopediaMenuClass::On_Init_Dialog (void)
{
	...
	if (!IS_MISSION) {
		DialogControlClass *load_button = Get_Dlg_Item (IDC_MENU_LOAD_SP_GAME_BUTTON);
		if (load_button != NULL) {
			load_button->Show (false);
			load_button->Enable (false);
		}
		DialogControlClass *save_button = Get_Dlg_Item (IDC_MENU_SAVE_SP_GAME_BUTTON);
		if (save_button != NULL) {
			save_button->Show (false);
	...
	DialogControlClass *suicide_button = Get_Dlg_Item (IDC_OPTIONS_MULTIPLAY_SUICIDE);
	if (suicide_button != NULL) {
		suicide_button->Enable (is_suicide_enabled);
		suicide_button->Show (is_suicide_enabled);
	}
	...
	DialogControlClass *change_teams_button = Get_Dlg_Item (IDC_OPTIONS_MULTIPLAY_CHANGE_TEAMS);
	if (change_teams_button != NULL) {
		change_teams_button->Enable (is_team_change_enabled);
		change_teams_button->Show (is_team_change_enabled);
	}
