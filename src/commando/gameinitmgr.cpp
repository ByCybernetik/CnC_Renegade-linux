...
GameInitMgrClass::Start_Game (const char *map_name, int teamChoice, unsigned long clanID)
{
   ...
   WWDEBUG_SAY (("GameInitMgrClass::Start_Game(%s)\n", map_name));
	...
	CampaignManager::Select_Backdrop_Number(
		cGameData::Get_Mission_Number_From_Map_Name( map.Peek_Buffer() ) );
   ...
   //
	...
	::_splitpath(map.Peek_Buffer(), NULL, NULL, NULL, extension);
	const bool is_save_game = (_stricmp(extension, ".sav") == 0);
	if (!is_save_game) {
...
GameInitMgrClass::Start_Client_Server (void)
{
   WWDEBUG_SAY (("GameInitMgrClass::Start_Client_Server\n"));
	...
	const bool lan_listen_host =
		GameModeManager::Find("LAN")->Is_Active() &&
		IsClientRequired &&
		IsServerRequired &&
	...
	if (lan_listen_host) {
		// Local host client uses in-process transport; server keeps UDP for LAN discovery/join.
		cSinglePlayerData::Enable_Lan_Host_Local_Client(true);
	}
	...
	if (IsClientRequired) {
		return (cNetwork::I_Am_Client () &&
				cNetwork::PClientConnection != NULL &&
...
GameInitMgrClass::Shutdown (void)
{
   WWDEBUG_SAY (("GameInitMgrClass::Shutdown\n"));
	...
	if (cSinglePlayerData::Is_Single_Player() || cSinglePlayerData::Is_Lan_Host_Local_Client()) {
		//
...
void _reload_game_configuration_files(void)
{
	...
	// Reload scripts.dll (MinGW/Wine: avoid FreeLibrary+LoadLibrary — breaks static registration)
	ScriptManager::Rebind_Script_Dll();
}
