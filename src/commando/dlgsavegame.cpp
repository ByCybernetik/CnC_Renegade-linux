...
static const char *SAVE_GAME_DIR = "Data\\save\\";
static const char *SAVE_GAME_FIND = "Data\\save\\*.sav";
#else
static const char *SAVE_GAME_DIR = "data\\save\\";
static const char *SAVE_GAME_FIND = "data\\save\\*.sav";
#endif
	...
	if (item_index != -1) {
		...
		if (list_ctrl->Get_Entry_Data (item_index, 0) != NULL) {
			...
			if (prompt) {
				...
				//
				StringClass full_path = SAVE_GAME_DIR;
				full_path += filename;
...
SaveGameMenuClass::Reload_List (const char *current_filename)
{
	...
	//
	int index = 1;
	for (file_find = ::FindFirstFile (SAVE_GAME_FIND, &find_info);
