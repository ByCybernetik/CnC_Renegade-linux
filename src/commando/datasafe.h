...
#define DECLARE_DATA_SAFE(type) DataSafeClass<type> DataSafe##type;
#else
...
unsigned long DataSafeClass<T>::Get_Type_Code(void)
{
	...
	*/
	...
	instruction_pointer = (unsigned long)(void*)&Get_Type_Code;
#else
...
inline SafeDataClass<T>::operator float (void) const
{
	...
	*/
	if (Handle.Is_Valid()) {
		bool ok = DataSafeClass<T>::Get(Handle, data_ptr);
