...
#if defined(RENEGADE_LINUX)
	const unsigned long drain_time = 100;
#else
	const unsigned long drain_time = servicetime;
#endif
	time = TIMEGETTIME();
	while (TIMEGETTIME() - time < drain_time) {
