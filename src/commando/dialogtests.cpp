...
MPJoinMenuClass::Update_Game_List (void)
{
	...
	for (objnode = cGameChannelList::Get_Chan_List()->Head(); objnode; objnode = objnode->Next()) {
	   ...
	   if (GameModeManager::Find("WOL")->Is_Active()) {
			RefPtr<WWOnline::ChannelData>& wol_channel = p_channel->Get_Channel();
			WWASSERT(wol_channel.IsValid());
			p_channel->Get_Game_Data()->Set_Current_Players(wol_channel->GetData().currentUsers);
		}		
		...
		if (item_index >= 0) {
			...
			list_ctrl->Set_Entry_Data (item_index, 0, (uintptr_t)p_channel);
		}
