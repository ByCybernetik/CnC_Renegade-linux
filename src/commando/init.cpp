...
*/
#if defined(RENEGADE_LINUX)
void Game_Init_Failed_Cleanup(void)
{
	if (WW3D::Is_Initted()) {
		WW3D::Shutdown();
	}
...
#if defined(RENEGADE_LINUX)
	/*
	** Unpacked installs (e.g. Other/ren/) keep assets in the game root as well as Data/.
	** Search the working directory first, then Data\save\config.
	*/
	RenegadeBaseFileFactory.Set_Sub_Directory( "" );
