...
/* No slop at static init — avoids heap traffic before main (Linux port). */
DataSafeClass<int> DataSafeint((int *)NULL, 0);
DataSafeClass<DATASAFE_UNSIGNED_INT> DataSafeDATASAFE_UNSIGNED_INT((DATASAFE_UNSIGNED_INT *)NULL, 0);
DataSafeClass<float> DataSafefloat((float *)NULL, 0);
DataSafeClass<double> DataSafedouble((double *)NULL, 0);
#else
...
#define DEFINE_DATA_SAFE_STATIC(type) \
template<> int DataSafeClass<type>::Type = 0; \
template<> char DataSafeClass<type>::ReturnList[MAX_OBJECT_COPIES][sizeof(type)] = { {0} }; \
template<> int DataSafeClass<type>::ReturnIndex = 0; \
template<> int DataSafeClass<type>::MinSlop = 0;

DEFINE_DATA_SAFE_STATIC(int);
DEFINE_DATA_SAFE_STATIC(DATASAFE_UNSIGNED_INT);
DEFINE_DATA_SAFE_STATIC(float);
DEFINE_DATA_SAFE_STATIC(double);
#endif
...
DataSafeEntryClass *GenericDataSafeClass::Get_Entry(DataSafeHandleClass handle)
{
	...
	** MinGW: do not call Security_Check here — it uses a static reentrancy flag that GCC
	** merges into Get_Entry and produces a broken fast-path (NULL Safe[list] deref).
	*/
