...
#if defined(RENEGADE_LINUX)
static bool movie_skip_input_hit (void)
{
	if (Input::Peek_State (INPUT_FUNCTION_MENU_TOGGLE)) {
		return true;
	}

	const char esc = DirectInput::Get_Keyboard_Button (DIK_ESCAPE);
	if ((esc & DirectInput::DI_BUTTON_HIT) != 0) {
		return true;
	}
...
void 	MovieGameModeClass::Think()
{
	...
	if ( Is_Active() ) {
		...
		bool leave = false;
#if defined(RENEGADE_LINUX)
		leave = movie_skip_input_hit ();
#else
		leave = Input::Get_State (INPUT_FUNCTION_MENU_TOGGLE);
#endif
...
void	MovieGameModeClass::Start_Movie( const char * filename )
{
	...
	//
	const DWORD attrs = ::GetFileAttributes (filename);
	if ( attrs != 0xFFFFFFFF ) {
		Play_Movie ( filename );
	} else {
