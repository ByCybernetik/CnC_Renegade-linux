...
#if defined(RENEGADE_LINUX)
		const bool data_ok =
			(access("Data/always.dat", F_OK) == 0) || (access("data/always.dat", F_OK) == 0);
		if (!data_ok) {
...
#if defined(RENEGADE_LINUX)
	(void)nCmdShow;
	MainWindow = Platform_Get_Main_HWnd();
	hWndMain = MainWindow;
 ...
 *=============================================================================================*/
#if defined(RENEGADE_LINUX)
static bool Linux_Has_Always_Dat_In(const char *dir)
{
	char path[512];
	if (dir == NULL || dir[0] == '\0') {
		return (access("Data/always.dat", F_OK) == 0) || (access("data/always.dat", F_OK) == 0);
	}
	snprintf(path, sizeof(path), "%sData/always.dat", dir);
