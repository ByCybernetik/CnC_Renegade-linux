	...
	MapCycleIndex(0)
{
   ...
   //Set_Owner(							"UNOWNED");
   {
		WideStringClass owner(cNetInterface::Get_Nickname(), true);
		Set_Owner(owner);
   }
...
static const WCHAR *Lookup_Host_Opt_Error(int id)
{
	const WCHAR *translated = TRANSLATE(id);
	if (translated != NULL && wcscmp(translated, L"TDBERR") != 0) {
		return translated;
	}

	switch (id) {
	case IDS_HOPTERR_NO_LEVEL:
		return L"Add at least one map to the map cycle list.";
	case IDS_HOPTERR_MAP_NOTFOUND:
		return L"Map file not found: %s";
	case IDS_HOPTERR_BLANK_PASS:
		return L"Password is enabled but empty.";
	case IDS_HOPTERR_QM_HASPASS:
		return L"Quick Match servers cannot use a password.";
	case IDS_HOPTERR_QM_NOLADDER:
		return L"Quick Match servers must be laddered.";
	case IDS_HOPTERR_QM_NOCLANS:
		return L"Quick Match servers cannot be clan games.";
	case IDS_HOPTERR_NO_TEAMCHANGE:
		return L"Team changing is not allowed with these settings.";
	case IDS_HOPTERR_MAXPLAYER_0:
		return L"Maximum players must be greater than zero.";
	case IDS_HOPTERR_MAXPLAYER_TOOBIG:
		return L"Maximum players is too high (max %d).";
	case IDS_HOPTERR_NO_MAPCYCLE:
		return L"Map cycling is enabled but the map cycle list is empty.";
	case IDS_HOPTERR_NO_GAMEEND:
		return L"Set a time limit or enable base destruction as a victory condition.";
	case IDS_HOPTERR_MOTD_TOOBIG:
		return L"Message of the day is too long (max %d characters).";
	case IDS_HOPTERR_TITLE_TOOBIG:
		return L"Game title is too long (max %d characters).";
	default:
		return L"Invalid server settings.";
	}
}
...
bool cGameData::Is_Valid_Settings(WideStringClass& outMsg, bool check_as_server)
{
	if (MapName.Is_Empty()) {
		...
		outMsg = Lookup_Host_Opt_Error(IDS_HOPTERR_NO_LEVEL);
		return false;
	} else {
		...
		if (!Is_Map_Valid(&filename)) {
			...
			outMsg.Format(Lookup_Host_Opt_Error(IDS_HOPTERR_MAP_NOTFOUND), filename);
			return(false);
		}
	}
	...
	if (IsPassworded.Is_True() && Password.Get_Length () == 0) {
		...
		outMsg = Lookup_Host_Opt_Error(IDS_HOPTERR_BLANK_PASS);
		return false;
	}
	...
	if (GameModeManager::Find("WOL")->Is_Active()) {
		...
		if (IsPassworded.Is_True() && Is_QuickMatch_Server()) {
			...
			outMsg = Lookup_Host_Opt_Error(IDS_HOPTERR_QM_HASPASS);
			return false;
		}
		...
		if (Is_QuickMatch_Server() && IsLaddered.Is_False()) {
			...
			outMsg = Lookup_Host_Opt_Error(IDS_HOPTERR_QM_NOLADDER);
			return false;
		}
		...
		if (Is_QuickMatch_Server() && IsClanGame.Is_True()) {
			...
			outMsg = Lookup_Host_Opt_Error(IDS_HOPTERR_QM_NOCLANS);
			return false;
		}
		...
		if (IsLaddered.Is_True() && IsTeamChangingAllowed.Is_True()) {
			...
			outMsg = Lookup_Host_Opt_Error(IDS_HOPTERR_NO_TEAMCHANGE);
			return false;
		}
	...
	if (Get_Max_Players() == 0) {
		...
		outMsg = Lookup_Host_Opt_Error(IDS_HOPTERR_MAXPLAYER_0);
		return(false);
	}
	...
	if (check_as_server || ConsoleBox.Is_Exclusive()) {
		if (Get_Max_Players() > NetworkObjectClass::MAX_CLIENT_COUNT-1) {
			...
			outMsg.Format(Lookup_Host_Opt_Error(IDS_HOPTERR_MAXPLAYER_TOOBIG), NetworkObjectClass::MAX_CLIENT_COUNT);
			return(false);
		}
		...
		if (DoMapsLoop && Get_Map_Cycle(0).Get_Length() == 0) {
			...
			outMsg = Lookup_Host_Opt_Error(IDS_HOPTERR_NO_MAPCYCLE);
			return(false);
		}
		...
		*/
		if (!Is_Limited()) {
			...
			outMsg = Lookup_Host_Opt_Error(IDS_HOPTERR_NO_GAMEEND);
			return false;
		}
		...
		*/
		if (Motd.Get_Length() > MAX_MOTD_LENGTH) {
			...
			outMsg.Format(Lookup_Host_Opt_Error(IDS_HOPTERR_MOTD_TOOBIG), MAX_MOTD_LENGTH);
			return false;
		}
		...
		*/
		if (GameTitle.Get_Length() > 100) {
			...
			outMsg.Format(Lookup_Host_Opt_Error(IDS_HOPTERR_TITLE_TOOBIG), 100);
			return false;
		}
	}
