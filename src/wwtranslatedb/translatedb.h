...
public:
	...
	static TDBObjClass *			Find_Object_Linear (const char *id_desc);
...
inline const WCHAR *TranslateDB_Safe_WChar_Ptr(const WCHAR *string)
{
	if (string == NULL || (unsigned long)(uintptr_t)string <= 1UL) {
...
TranslateDBClass::Get_String (uint32 id)
{
	...
	return TranslateDB_Safe_WChar_Ptr(string);
}
...
TranslateDBClass::Get_String (const char *id_desc)
{
	...
	if (translate_obj == NULL) {
		translate_obj = Find_Object_Linear (id_desc);
	}
	if (translate_obj != NULL) {
