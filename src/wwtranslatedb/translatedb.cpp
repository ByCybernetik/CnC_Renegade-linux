...
DECLARE_FORCE_LINK(TranslateDB)

void Force_Link_WWTranslateDB(void)
{
	FORCE_LINK(TranslateDB);
	(void)&_TheTranslateDB;
	extern SimplePersistFactoryClass<TDBObjClass, CHUNKID_TRANSLATE_OBJ>
		_TranslateObjPersistFactory;
	extern SimplePersistFactoryClass<TDBCategoryClass, CHUNKID_TDBCATEGORY>
		_TDBCategoryPersistFactory;
	(void)&_TranslateObjPersistFactory;
	(void)&_TDBCategoryPersistFactory;
	return;
}


TranslateDBClass::TranslateDBClass(void) : SaveLoadSubSystemClass()
{
}


TranslateDBClass::~TranslateDBClass(void)
{
}
...
{
	_TheTranslateDB.Ensure_Registered (CHUNKID_TRANSLATE_DB);
...
///////////////////////////////////////////////////////////////////////
TDBObjClass *
TranslateDBClass::Find_Object_Linear (const char *id_desc)
{
	if (id_desc == NULL) {
		return NULL;
	}

	StringClass want (id_desc, true);
	_strlwr (want.Peek_Buffer ());
	...
	for (int index = 0; index < m_ObjectList.Count (); index++) {
		...
		StringClass desc (object->Get_ID_Desc (), true);
		_strlwr (desc.Peek_Buffer ());
		if (desc == want) {
			return object;
		}
	}
