...
class BitStreamClass : public cBitPacker
{
	public:
      ...
      void Add_Wide_Terminated_String(const WCHAR *string, bool permit_empty = false);
		...
		void		Add(unsigned long val,int type = NO_ENCODER)			{ Internal_Add(static_cast<uint32_t>(val),type); }
#else
...
#if defined(RENEGADE_LINUX)
		UINT		Get(unsigned long & set_val,int type = NO_ENCODER)	{
			uint32_t tmp = 0;
			UINT r = Internal_Get(tmp, type);
			set_val = tmp;
			return r;
		}
#else
