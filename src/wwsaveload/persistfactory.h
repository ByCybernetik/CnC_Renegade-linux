...
SimplePersistFactoryClass<T,CHUNKID>::Load(ChunkLoadClass & cload) const 
{
	...
	/* Win32 save files store object pointers as 32-bit values. */
	{
		uint32 old_obj_u32 = 0;
		cload.Read(&old_obj_u32, sizeof(old_obj_u32));
		old_obj = reinterpret_cast<T *>(uintptr_t(old_obj_u32));
	}
#else
