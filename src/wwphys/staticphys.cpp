...
bool StaticPhysClass::Cast_OBBox ( PhysOBBoxCollisionTestClass &boxtest )
{
	WWASSERT ( Model );
	...
	/*
	** Mesh triangle casts can fail on LP64 while the cull AABox is still valid.
	** Use the cull volume for swept movement blocking on compact static props only.
	*/
	if ( boxtest.Move.Length2 () > WWMATH_EPSILON && !boxtest.Result->StartBad ) {
		const AABoxClass &cull = Get_Cull_Box ();
		float max_ext = cull.Extent.X;
		...
		if (	max_ext >= 0.25f && max_ext <= 10.0f &&
				( class_id == RenderObjClass::CLASSID_MESH || class_id == RenderObjClass::CLASSID_HLOD ) )
		{
			if (	CollisionMath::Collide ( boxtest.Box, boxtest.Move, cull, Vector3 ( 0, 0, 0 ), boxtest.Result ) &&
					!boxtest.Result->StartBad &&
					boxtest.Result->Fraction > WWMATH_EPSILON )
			{
				boxtest.CollidedPhysObj = this;
				return true;
			}
