...
void DynamicShadowTexMgrClass::Set_Max_Simultaneous_Shadows(unsigned int max)
{
	...
	for (int i=curlen; i<ShadowTextures.Length(); i++) {
		ShadowTextures[i] = Allocate_Render_Target_Texture();
	}
}
...
#if defined(RENEGADE_VULKAN)
		/* Vulkan render targets have no IDirect3DSurface8; skip D3D8 readback probe. */
	...
	if (existing_texture == NULL) {
...
#if defined(RENEGADE_VULKAN)
		TextureClass * new_texture = ww3d_vulkan::Try_Clone_Render_Target_Texture(render_target);
		shadow_projector->Set_Render_Target(NULL);
		if (new_texture != NULL) {
			shadow_projector->Set_Texture(new_texture);
			_StaticShadowTexMgr.Add_Shadow_Texture(
				type_id,
				obj_orientation,
				shadow_projector->Peek_Texture());
		...
		if (surf != NULL) {
			...
			DX8Wrapper::_Copy_DX8_Rects(
				surf->Peek_D3D_Surface(),
				NULL,
				0,
				new_surf->Peek_D3D_Surface(),
				NULL);
