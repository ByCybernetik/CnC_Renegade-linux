...
void PhysicsSceneClass::Post_Load_Level_Static_Objects(void)
{
	...
	//
	//	Rebuild static collision tree from object bounds.  Saved AAB-tree
	//	linkage from level files is unreliable on LP64 and lets vehicles
	//	pass through poles, walls, and other static geometry.
	//
	if (StaticCullingSystem != NULL) {
		RefPhysListIterator it(&StaticObjList);
		for (it.First(); !it.Is_Done(); it.Next()) {
			...
			StaticCullingSystem->Add_Object(obj, -1);
		}
		StaticCullingSystem->Re_Partition();

		//
		// Saved PVS tables index AAB-tree nodes from the level file.  After
		// rebuilding the tree those node IDs are stale and hierarchical vis
		// culling rejects every cell — use frustum culling until vis is
		// regenerated (editor export or runtime vis bake).
		//
		VisTableManager.Reset();
		LastValidVisId = -1;
	}
...
void PhysicsSceneClass::Load_Static_Object_States(ChunkLoadClass & cload)
{
	while (cload.Open_Chunk()) {
		...
		void * old_ptr = (void *)(uintptr_t)old_ptr_u32;
#else
		void * old_ptr;
