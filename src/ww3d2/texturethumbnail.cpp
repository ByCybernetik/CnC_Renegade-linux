...
#if defined(RENEGADE_LINUX)
	/*
	** Skip .thu cache I/O on Linux (see Update_Thumbnail_File). Textures load via
	** MissingTexture / direct load; parsing always.thu from game data can SEGV
	** when the cache was built on Win32 or file I/O differs on LP64.
	*/
	Changed = false;
	return;
#endif
...
#if defined(RENEGADE_LINUX)
	/*
	** Building .thu from always.dat (~550MB) blocks startup for a long time on first
	** launch. Textures still load; missing thumbnails use the placeholder path.
	*/
	mix_file->Close();
	thumb_file->Close();
	ff.Return_File(mix_file);
