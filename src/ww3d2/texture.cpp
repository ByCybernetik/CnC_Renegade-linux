...
#include "../ww3d2_vulkan/ww3d_vulkan.h"
#endif
	...
	LastInactivationSyncTime(0)
{
	...
	if (DX8Wrapper::Vulkan_Device_Active()) {
		if (rendertarget) {
			ww3d_vulkan::VkTexture *vk_tex = ww3d_vulkan::Try_Create_Render_Target(width, height, format);
			if (vk_tex != NULL) {
				Set_Vulkan_Texture(vk_tex);
				Width = (int)vk_tex->Width();
				Height = (int)vk_tex->Height();
			}
		}
		/* Non-render-target procedural textures: Vulkan_Texture is set by the caller. */
	} else {
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active() && surface->Peek_Vulkan_Cpu_Surface() != nullptr) {
...
TextureClass::~TextureClass(void)
{
	...
	if (Vulkan_Texture != nullptr) {
		ww3d_vulkan::VkTexture *vk_tex = static_cast<ww3d_vulkan::VkTexture *>(Vulkan_Texture);
		vk_tex->Destroy();
		delete vk_tex;
		Vulkan_Texture = nullptr;
	}
#endif
...
void TextureClass::Apply(unsigned int stage)
{
	...
	if (DX8Wrapper::Vulkan_Device_Active()) {
		if (WW3D::Is_Texturing_Enabled()) {
			ww3d_vulkan::Texture_Stage_Bind(this, stage);
		} else {
			ww3d_vulkan::Texture_Stage_Bind_Null(stage);
		}
		return;
	}
#endif
