...
////////////////////////////////////////////////////////////////////////////////////
void
Render2DSentenceClass::Reset_Sentence_Data (bool release_surfaces)
{
	if (release_surfaces) {
...
Render2DSentenceClass::Build_Textures (void)
{
	...
	//
	//	Upload one GPU texture per renderer/atlas (Renderers list is authoritative).
	//	PendingSurfaces.Renderers linkage can miss reused renderers after Reset_Active.
	//
	for (int index = 0; index < Renderers.Count (); index ++) {
		SurfaceClass *curr_surface = Renderers[index].Surface;
		Render2DClass *renderer = Renderers[index].Renderer;
		if (curr_surface == NULL || renderer == NULL) {
			continue;
		}
...
#if defined(RENEGADE_VULKAN)
void
Render2DSentenceClass::Ensure_Pending_Surface (SurfaceClass *surface, Render2DClass *renderer)
{
	if (surface == NULL || renderer == NULL) {
		return;
	}
	for (int i = 0; i < PendingSurfaces.Count(); ++i) {
		if (PendingSurfaces[i].Surface == surface) {
			bool has_renderer = false;
			for (int j = 0; j < PendingSurfaces[i].Renderers.Count(); ++j) {
				if (PendingSurfaces[i].Renderers[j] == renderer) {
					has_renderer = true;
					break;
				}
			}
			if (!has_renderer) {
				PendingSurfaces[i].Renderers.Add(renderer);
			}
			return;
		}
	}
	...
	surface_info.Surface->Add_Ref();
	surface_info.Renderers.Add(renderer);
...
#if defined(RENEGADE_VULKAN)
			Ensure_Pending_Surface (curr_surface, curr_renderer);
#endif
		}
...
Render2DSentenceClass::Build_Sentence (const WCHAR *text)
{
	...
	//
	//	Drop the previous chunk list but keep atlas surfaces alive for quads
	//	already submitted via Draw_Sentence.
	//
	Reset_Sentence_Data (false);
	Cursor.Set (0, 0);
	...
	if (CurSurface == NULL) {
		...
		//
		//	Anchor the next sentence to fresh atlas pixels. Without this,
		//	UV width spans the entire atlas (0..TextureOffset) and every
		//	draw shows all previously baked glyphs.
		//
	...
	//
	//	Record the last run of characters (MinGW relied on processing L'\0'
	// inside the loop; the Linux loop exits before that sentinel is seen).
	//
	Record_Sentence_Chunk ();
...
#if defined(RENEGADE_LINUX)
	::memset( UnicodePages, 0, sizeof (UnicodePages) );
#endif
	return ;
}
...
#if defined(RENEGADE_LINUX)
	const unsigned short uch = (unsigned short)ch;

	if ( uch < 256u ) {
		retval = ASCIICharArray[uch];
	} else {
		const unsigned page = uch >> 8;
		const unsigned slot = uch & 0xFFu;
		if ( UnicodePages[page] != NULL ) {
			retval = UnicodePages[page][slot];
		}
	}
#else
	if ( ch < 256 ) {
...
{
#if defined(RENEGADE_LINUX)
	if ( MemDC == NULL || GDIBitmapBits == NULL || GDIFont == NULL ) {
		return NULL;
	}
#endif
...
#if defined(RENEGADE_LINUX)
	const int copy_rows = MIN( char_size.cy, height );
	const int copy_cols = MIN( char_size.cx, width );
	...
	for (int row = 0; row < char_size.cy; row ++) {
		...
		for (int col = 0; col < char_size.cx; col ++) {
			...
			//
#if defined(RENEGADE_LINUX)
			uint8 alpha_value = (uint8)((pixel_value * 15 + 127) / 255);
#else
	...
	bool stored = false;
	if ( uch < 256u ) {
		ASCIICharArray[uch] = char_data;
		stored = true;
	} else {
		const unsigned page = uch >> 8;
		const unsigned slot = uch & 0xFFu;
		if ( UnicodePages[page] == NULL ) {
			CharDataStruct **page_array = new CharDataStruct *[256];
			::memset( page_array, 0, sizeof (CharDataStruct *) * 256 );
			UnicodePages[page] = page_array;
		}
		UnicodePages[page][slot] = char_data;
		stored = true;
	}
...
FontCharsClass::Create_GDI_Font (const char *font_name)
{
...

	{
		...
		int try_count = 0;
		try_names[try_count++] = font_name;
		if (font_name != NULL && ::strstr(font_name, "Arial MT") != NULL) {
			try_names[try_count++] = "Arial";
		}
		if (font_name != NULL && ::strstr(font_name, "Regatta") != NULL) {
			try_names[try_count++] = "Arial";
		}
		try_names[try_count++] = "Tahoma";

		for (int ti = 0; ti < try_count && GDIFont == NULL; ti++) {
			GDIFont = ::CreateFont(
				font_height,
				0,
				0,
				...
				VARIABLE_PITCH,
				try_names[ti]);
			if (GDIFont != NULL && try_names[ti] != font_name) {
				GDIFontName = try_names[ti];
			}
		}
	}
	...
	for (int page = 0; page < 256; page ++) {
		if ( UnicodePages[page] != NULL ) {
			for (int slot = 0; slot < 256; slot ++) {
				if ( UnicodePages[page][slot] != NULL ) {
					delete UnicodePages[page][slot];
					UnicodePages[page][slot] = NULL;
				}
			}
			delete [] UnicodePages[page];
			UnicodePages[page] = NULL;
		}
	}
#else
	if ( UnicodeCharArray != NULL ) {
