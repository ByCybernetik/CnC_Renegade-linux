...
/* Must match DDSURFACEDESC2 size stored in retail .dds files (dwSize = 124). */
...
#if defined(RENEGADE_LINUX)
	unsigned read_bytes = file->Read(&SurfaceDesc, LEGACY_DDSURFACEDESC2_BYTES);
	if (read_bytes != LEGACY_DDSURFACEDESC2_BYTES
			|| SurfaceDesc.Size != LEGACY_DDSURFACEDESC2_BYTES) {
		return;
	}
#else
...
void DDSFileClass::Copy_Level_To_Surface(unsigned level,IDirect3DSurface8* d3d_surface)
{
	...
	DX8_ErrorCode(d3d_surface->LockRect(Renegade_AsD3DLockedRect(&locked_rect),NULL,0));
	...
	Copy_Level_To_Surface(
		...
		surface_desc.Width,
		surface_desc.Height,
		reinterpret_cast<unsigned char*>(Renegade_D3DLockedRect_PBits(locked_rect)),
		locked_rect.Pitch);
	...
	unsigned dest_pitch)
{
	...
	if (dest_format==Format && dest_width==Get_Width(level) && dest_height==Get_Height(level)) {
		const unsigned char *src = Get_Memory_Pointer(level);
		const unsigned compressed_size = Get_Level_Size(level);
		const unsigned blocks_wide = dest_width / 4;
		const unsigned block_rows = dest_height / 4;
		const unsigned block_bytes =
			(Format == WW3D_FORMAT_DXT1) ? 8u : 16u;
		const unsigned src_row_bytes = blocks_wide * block_bytes;

		if (src_row_bytes == 0 || block_rows == 0) {
			return;
		}

		// D3D/DXVK pitch is bytes per row of 4x4 blocks; may exceed tight size.
		if (dest_pitch == src_row_bytes) {
			memcpy(dest_surface, src, compressed_size);
		} else {
			for (unsigned row = 0; row < block_rows; ++row) {
				memcpy(
					dest_surface + row * dest_pitch,
