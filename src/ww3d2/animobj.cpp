...
void Animatable3DObjClass::Update_Sub_Object_Transforms(void)
{
	...
	*/
	switch (CurMotionMode) {
		...
		case DOUBLE_ANIM:
			if (ModeInterp.Motion0 != NULL && ModeInterp.Motion1 != NULL) {
			...
			} else if (ModeInterp.Motion0 != NULL) {
				Anim_Update(Transform, ModeInterp.Motion0, ModeInterp.Frame0);
			} else if (ModeInterp.Motion1 != NULL) {
				Anim_Update(Transform, ModeInterp.Motion1, ModeInterp.Frame1);
			} else {
				Base_Update(Transform);
			}
