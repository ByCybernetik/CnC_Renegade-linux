...
public:
	...
	*/
	...
	virtual HAnimClass *				Peek_HAnim(const char * name);
	virtual bool						Ensure_HTree_Loaded( const char * hierarchy_name );
	virtual bool						Ensure_W3D_File_Loaded( const char * asset_basename );
	virtual bool						Is_HAnim_Missing(const char * name);
	virtual void						Clear_HAnim_Missing(const char * name);
	virtual HAnimClass *				Resolve_HAnim(const char * name);
...
//	virtual AssetIterator *			Create_Texture_Iterator(void);
	...
	*/
	...
	virtual HTreeClass *				Peek_HTree(const char * name);
...
protected:
	...
	*/
	...
	void									load_nested_assets( ChunkLoadClass & cload );
	HAnimClass *						find_loaded_hanim( const char *name );
