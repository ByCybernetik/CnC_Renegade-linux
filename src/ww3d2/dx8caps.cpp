...
//	Init_Caps(D3DDevice);
	...
	int Product, Version, SubVersion;
#if defined(_WIN32)
	Product = HIWORD(adapter_id.DriverVersion.HighPart);
	Version = LOWORD(adapter_id.DriverVersion.HighPart);
	SubVersion = HIWORD(adapter_id.DriverVersion.LowPart);
	DriverBuildVersion = LOWORD(adapter_id.DriverVersion.LowPart);
#else
	Product = (int)((adapter_id.DriverVersionHighPart >> 16) & 0xFFFF);
	Version = (int)(adapter_id.DriverVersionHighPart & 0xFFFF);
	SubVersion = (int)((adapter_id.DriverVersionLowPart >> 16) & 0xFFFF);
	DriverBuildVersion = (int)(adapter_id.DriverVersionLowPart & 0xFFFF);
#endif
...
static bool Vulkan_Supports_Texture_Format(WW3DFormat format)
{
	switch (format) {
...
static bool Vulkan_Supports_Render_To_Texture_Format(WW3DFormat format)
{
	return Vulkan_Supports_Texture_Format(format);
}
#endif
	...
	if (Direct3D == NULL) {
		for (unsigned i = 0; i < WW3D_FORMAT_COUNT; ++i) {
			if (i == WW3D_FORMAT_UNKNOWN) {
				...
				SupportTextureFormat[i] = Vulkan_Supports_Texture_Format((WW3DFormat)i);
			}
		}
		return;
	}
	...
	if (Direct3D == NULL) {
		for (unsigned i = 0; i < WW3D_FORMAT_COUNT; ++i) {
			if (i == WW3D_FORMAT_UNKNOWN) {
				...
				SupportRenderToTextureFormat[i] =
					Vulkan_Supports_Render_To_Texture_Format((WW3DFormat)i);
			}
		}
		return;
	}
