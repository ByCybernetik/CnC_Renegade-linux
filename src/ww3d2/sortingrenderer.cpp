...
static void Sorting_Draw_Triangles(
	...
	unsigned short vertex_count)
{
	...
	DX8Wrapper::Get_Transform(D3DTS_PROJECTION, state->sorting_state.projection);
	...
	Matrix4 world_view = state->sorting_state.world * state->sorting_state.view;
	Vector3 vec = state->bounding_sphere.Center;
	Vector4 transformed_vec = world_view * Vector4(vec.X, vec.Y, vec.Z, 1.0f);
...
void SortingRendererClass::Flush_Sorting_Pool()
{
	...
	DynamicVBAccessClass dyn_vb_access(BUFFER_TYPE_DYNAMIC_DX8,dynamic_fvf_type,overlapping_vertex_count);
	{
		...
		for (node_id=0;node_id<overlapping_node_count;++node_id) {
			...
			Matrix4 mtx = (state->sorting_state.world * state->sorting_state.view).Transpose();
	...
	for (unsigned i=1;i<overlapping_polygon_count;++i) {
		if (node_id!=tis[i].idx) {
			...
			Sorting_Draw_Triangles(
				start_index * 3,
				(unsigned short)count_to_render,
