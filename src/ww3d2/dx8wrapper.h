...
{
#if defined(RENEGADE_VULKAN)
	friend void ww3d_vulkan::Fill_Vulkan_Draw_State(ww3d_vulkan::VulkanDrawState *);
#endif
	enum ChangedStates {
		WORLD_CHANGED	=	1<<0,
		VIEW_CHANGED	=	1<<1,
...
public:
	...
	*/
	...
	static int Get_Effective_ZBias();
	static float Get_ZNear_For_ZBias();
	static float Get_ZFar_For_ZBias();
...
#if defined(RENEGADE_VULKAN)
	static float					Get_Fog_Start() { return FogStart; }
	static float					Get_Fog_End() { return FogEnd; }
	static unsigned					Get_Scene_Ambient_Color() { return RenderStates[D3DRS_AMBIENT]; }
#endif
	...
	*/
	...
	static void Record_Texture_Matrix(unsigned stage);
#if defined(RENEGADE_VULKAN)
	static void Vulkan_Notify_Frame_End(bool present, bool present_ok);
	static void Vulkan_Reset_Render_State_After_Frame();
	static void Vulkan_Draw_Triangles(
		unsigned short start_index,
		unsigned short polygon_count,
		unsigned short min_vertex_index,
		unsigned short vertex_count);
	static void Vulkan_Draw_Strip(
		unsigned short start_index,
		unsigned short polygon_count,
		unsigned short min_vertex_index,
		unsigned short vertex_count);
...
#if defined(RENEGADE_VULKAN)
	static void Enumerate_Vulkan_Devices();
#endif
...
#if defined(RENEGADE_VULKAN)
	Record_Texture_Stage_State(stage, (unsigned)state, value);
#endif
...
#if !defined(_MSC_VER)
	unsigned int a = (unsigned int)(alpha * scale);
	unsigned int r = (unsigned int)(color[0] * scale);
	unsigned int g = (unsigned int)(color[1] * scale);
	unsigned int b = (unsigned int)(color[2] * scale);
...
WWINLINE int DX8Wrapper::Get_Effective_ZBias()
{
	int effective_zbias = ZBias;
	const unsigned rs_zbias = RenderStates[D3DRS_ZBIAS];
	if (rs_zbias > (unsigned)effective_zbias) {
		effective_zbias = (int)rs_zbias;
	}
	if (effective_zbias > 15) {
		effective_zbias = 15;
	}
	if (effective_zbias < 0) {
		effective_zbias = 0;
	}
	return effective_zbias;
}

WWINLINE float DX8Wrapper::Get_ZNear_For_ZBias()
{
	return ZNear;
}

WWINLINE float DX8Wrapper::Get_ZFar_For_ZBias()
{
	return ZFar;
}
...
#if defined(RENEGADE_VULKAN)
		if (Vulkan_Device_Active() &&
			transform >= D3DTS_TEXTURE0 &&
			transform < D3DTS_TEXTURE0 + MAX_TEXTURE_STAGES) {
			unsigned stage = (unsigned)(transform - D3DTS_TEXTURE0);
			TextureMatrices[stage] = m.Transpose();
