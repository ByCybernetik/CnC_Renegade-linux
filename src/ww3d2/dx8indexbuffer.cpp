...
#if defined(RENEGADE_VULKAN)
#define VULKAN_DEFAULT_IB_SIZE 16384
static constexpr uint32_t kMaxRetiredVulkanIbPerSlot = 32;
#endif
...
#if defined(RENEGADE_VULKAN)
static DX8IndexBufferClass *_VulkanDynamicIB[ww3d_vulkan::kMaxFramesInFlight] = {};
static unsigned short _VulkanDynamicIBSize[ww3d_vulkan::kMaxFramesInFlight] = {};
static unsigned short _VulkanDynamicIBOffset[ww3d_vulkan::kMaxFramesInFlight] = {};
static bool _VulkanDynamicIBInUse[ww3d_vulkan::kMaxFramesInFlight] = {};
static DX8IndexBufferClass *_VulkanDynamicIBRetired[ww3d_vulkan::kMaxFramesInFlight]
	[kMaxRetiredVulkanIbPerSlot] = {};
static uint32_t _VulkanDynamicIBRetiredCount[ww3d_vulkan::kMaxFramesInFlight] = {};

static void Retire_Vulkan_Dynamic_Ib(uint32_t slot, DX8IndexBufferClass *ib)
{
	if (ib == nullptr) {
		return;
	}
	WWASSERT(slot < ww3d_vulkan::kMaxFramesInFlight);
	WWASSERT(_VulkanDynamicIBRetiredCount[slot] < kMaxRetiredVulkanIbPerSlot);
	_VulkanDynamicIBRetired[slot][_VulkanDynamicIBRetiredCount[slot]++] = ib;
}

static void Release_Retired_Vulkan_Dynamic_Ibs(uint32_t slot)
{
	for (uint32_t i = 0; i < _VulkanDynamicIBRetiredCount[slot]; ++i) {
		REF_PTR_RELEASE(_VulkanDynamicIBRetired[slot][i]);
		_VulkanDynamicIBRetired[slot][i] = nullptr;
	}
	_VulkanDynamicIBRetiredCount[slot] = 0;
}
#endif
...
IndexBufferClass::WriteLockClass::WriteLockClass(IndexBufferClass* index_buffer_) : index_buffer(index_buffer_)
{
	...
	case BUFFER_TYPE_DX8:
		...
		if (DX8Wrapper::Vulkan_Device_Active()) {
			indices = reinterpret_cast<unsigned short *>(ww3d_vulkan::IB_Lock(
...
#if defined(RENEGADE_VULKAN)
		if (DX8Wrapper::Vulkan_Device_Active()) {
			ww3d_vulkan::IB_Unlock(static_cast<DX8IndexBufferClass *>(index_buffer));
		} else
#endif
		{
			DX8_Assert();
			DX8_ErrorCode(static_cast<DX8IndexBufferClass *>(index_buffer)->Get_DX8_Index_Buffer()->Unlock());
		}
		break;
	case BUFFER_TYPE_SORTING:
		break;
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		size_t bytes = sizeof(WORD) * index_count;
		if (!ww3d_vulkan::IB_Create(this, bytes)) {
			fprintf(stderr, "Vulkan: index buffer allocation failed (%zu bytes)\n", bytes);
			std::abort();
		}
		return;
	}
	...
	if (Type==BUFFER_TYPE_DYNAMIC_DX8) {
		...
		if (DX8Wrapper::Vulkan_Device_Active()) {
			...
			if (slot < ww3d_vulkan::kMaxFramesInFlight) {
				_VulkanDynamicIBInUse[slot] = false;
				_VulkanDynamicIBOffset[slot] += IndexCount;
			}
		} else
#endif
		{
...
void DynamicIBAccessClass::_Deinit()
{
	...
	for (uint32_t i = 0; i < ww3d_vulkan::kMaxFramesInFlight; ++i) {
		Release_Retired_Vulkan_Dynamic_Ibs(i);
		if (_VulkanDynamicIB[i] != nullptr) {
			WWASSERT(_VulkanDynamicIB[i]->Num_Refs() == 1);
		}
		REF_PTR_RELEASE(_VulkanDynamicIB[i]);
		_VulkanDynamicIBSize[i] = VULKAN_DEFAULT_IB_SIZE;
		_VulkanDynamicIBOffset[i] = 0;
		_VulkanDynamicIBInUse[i] = false;
	}
#endif
...
//		WWASSERT(!dynamic_dx8_index_buffer->Engine_Refs());
		...
		if (DX8Wrapper::Vulkan_Device_Active()) {
			Indices = reinterpret_cast<unsigned short *>(ww3d_vulkan::IB_Lock(
				static_cast<DX8IndexBufferClass *>(DynamicIBAccess->IndexBuffer),
...
#if defined(RENEGADE_VULKAN)
		if (DX8Wrapper::Vulkan_Device_Active()) {
			ww3d_vulkan::IB_Unlock(static_cast<DX8IndexBufferClass *>(DynamicIBAccess->IndexBuffer));
		} else
#endif
		{
			DX8_Assert();
...
void DynamicIBAccessClass::Allocate_DX8_Dynamic_Buffer()
{
	...
	if (DX8Wrapper::Vulkan_Device_Active()) {
		const uint32_t slot = ww3d_vulkan::Get_Dynamic_Vb_Frame_Slot();
		WWASSERT(slot < ww3d_vulkan::kMaxFramesInFlight);
		WWASSERT(!_VulkanDynamicIBInUse[slot]);
		_VulkanDynamicIBInUse[slot] = true;

		if (IndexCount > _VulkanDynamicIBSize[slot]) {
			Retire_Vulkan_Dynamic_Ib(slot, _VulkanDynamicIB[slot]);
			_VulkanDynamicIB[slot] = nullptr;
			_VulkanDynamicIBSize[slot] = IndexCount;
			if (_VulkanDynamicIBSize[slot] < VULKAN_DEFAULT_IB_SIZE) {
				_VulkanDynamicIBSize[slot] = VULKAN_DEFAULT_IB_SIZE;
			}
		}

		if (!_VulkanDynamicIB[slot]) {
			...
			_VulkanDynamicIB[slot] = NEW_REF(
				DX8IndexBufferClass,
				(_VulkanDynamicIBSize[slot],
				 (DX8IndexBufferClass::UsageType)usage));
			_VulkanDynamicIBOffset[slot] = 0;
		}

		if (((unsigned)IndexCount + _VulkanDynamicIBOffset[slot]) >
			_VulkanDynamicIBSize[slot]) {
			Retire_Vulkan_Dynamic_Ib(slot, _VulkanDynamicIB[slot]);
			_VulkanDynamicIB[slot] = nullptr;
			unsigned usage = DX8IndexBufferClass::USAGE_DYNAMIC;
		...
		REF_PTR_SET(IndexBuffer, _VulkanDynamicIB[slot]);
		IndexBufferOffset = _VulkanDynamicIBOffset[slot];
		return;
	}
#endif
...
#if defined(RENEGADE_VULKAN)
void DynamicIBAccessClass::_Reset_Vulkan_Frame_Slot(uint32_t slot)
{
	if (slot >= ww3d_vulkan::kMaxFramesInFlight) {
		return;
	}
	Release_Retired_Vulkan_Dynamic_Ibs(slot);
	_VulkanDynamicIBOffset[slot] = 0;
}
#endif
