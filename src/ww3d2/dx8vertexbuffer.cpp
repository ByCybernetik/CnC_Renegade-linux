...
#if defined(RENEGADE_VULKAN)
#define VULKAN_DEFAULT_VB_SIZE 16384
static constexpr uint32_t kMaxRetiredVulkanVbPerSlot = 32;
#endif
...
#if defined(RENEGADE_VULKAN)
static DX8VertexBufferClass *_VulkanDynamicVB[ww3d_vulkan::kMaxFramesInFlight] = {};
static unsigned short _VulkanDynamicVBSize[ww3d_vulkan::kMaxFramesInFlight] = {};
static unsigned short _VulkanDynamicVBOffset[ww3d_vulkan::kMaxFramesInFlight] = {};
static bool _VulkanDynamicVBInUse[ww3d_vulkan::kMaxFramesInFlight] = {};
static DX8VertexBufferClass *_VulkanDynamicVBRetired[ww3d_vulkan::kMaxFramesInFlight]
	[kMaxRetiredVulkanVbPerSlot] = {};
static uint32_t _VulkanDynamicVBRetiredCount[ww3d_vulkan::kMaxFramesInFlight] = {};

static void Retire_Vulkan_Dynamic_Vb(uint32_t slot, DX8VertexBufferClass *vb)
{
	if (vb == nullptr) {
		return;
	}
	WWASSERT(slot < ww3d_vulkan::kMaxFramesInFlight);
	WWASSERT(_VulkanDynamicVBRetiredCount[slot] < kMaxRetiredVulkanVbPerSlot);
	_VulkanDynamicVBRetired[slot][_VulkanDynamicVBRetiredCount[slot]++] = vb;
}

static void Release_Retired_Vulkan_Dynamic_Vbs(uint32_t slot)
{
	for (uint32_t i = 0; i < _VulkanDynamicVBRetiredCount[slot]; ++i) {
		REF_PTR_RELEASE(_VulkanDynamicVBRetired[slot][i]);
		_VulkanDynamicVBRetired[slot][i] = nullptr;
	}
	_VulkanDynamicVBRetiredCount[slot] = 0;
}
#endif
...
#if defined(RENEGADE_VULKAN)
		if (DX8Wrapper::Vulkan_Device_Active()) {
			ww3d_vulkan::VB_Unlock(static_cast<DX8VertexBufferClass *>(VertexBuffer));
		} else
#endif
		{
			DX8_Assert();
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		size_t bytes = FVF_Info().Get_FVF_Size() * VertexCount;
		if (!ww3d_vulkan::VB_Create(this, bytes)) {
			fprintf(stderr, "Vulkan: vertex buffer allocation failed (%zu bytes)\n", bytes);
			std::abort();
		}
		return;
	}
...
#if defined(RENEGADE_VULKAN)
		if (DX8Wrapper::Vulkan_Device_Active()) {
			const uint32_t slot = ww3d_vulkan::Get_Dynamic_Vb_Frame_Slot();
			if (slot < ww3d_vulkan::kMaxFramesInFlight) {
				_VulkanDynamicVBInUse[slot] = false;
				_VulkanDynamicVBOffset[slot] += VertexCount;
			}
		} else
#endif
		{
...
{
#if defined(RENEGADE_VULKAN)
	for (uint32_t i = 0; i < ww3d_vulkan::kMaxFramesInFlight; ++i) {
		Release_Retired_Vulkan_Dynamic_Vbs(i);
		if (_VulkanDynamicVB[i] != nullptr) {
			WWASSERT(_VulkanDynamicVB[i]->Num_Refs() == 1);
		}
		REF_PTR_RELEASE(_VulkanDynamicVB[i]);
		_VulkanDynamicVBSize[i] = VULKAN_DEFAULT_VB_SIZE;
		_VulkanDynamicVBOffset[i] = 0;
		_VulkanDynamicVBInUse[i] = false;
	}
#endif
...
void DynamicVBAccessClass::Allocate_DX8_Dynamic_Buffer()
{
	...
	if (DX8Wrapper::Vulkan_Device_Active()) {
		const uint32_t slot = ww3d_vulkan::Get_Dynamic_Vb_Frame_Slot();
		WWASSERT(slot < ww3d_vulkan::kMaxFramesInFlight);
		WWASSERT(!_VulkanDynamicVBInUse[slot]);
		_VulkanDynamicVBInUse[slot] = true;

		if (VertexCount > _VulkanDynamicVBSize[slot]) {
			Retire_Vulkan_Dynamic_Vb(slot, _VulkanDynamicVB[slot]);
			_VulkanDynamicVB[slot] = nullptr;
			_VulkanDynamicVBSize[slot] = VertexCount;
			if (_VulkanDynamicVBSize[slot] < VULKAN_DEFAULT_VB_SIZE) {
				_VulkanDynamicVBSize[slot] = VULKAN_DEFAULT_VB_SIZE;
			}
		}

		if (!_VulkanDynamicVB[slot]) {
			...
			_VulkanDynamicVB[slot] = NEW_REF(
				DX8VertexBufferClass,
				(dynamic_fvf_type,
				 _VulkanDynamicVBSize[slot],
				 (DX8VertexBufferClass::UsageType)usage));
			_VulkanDynamicVBOffset[slot] = 0;
		}

		if (((unsigned)VertexCount + _VulkanDynamicVBOffset[slot]) >
			_VulkanDynamicVBSize[slot]) {
			Retire_Vulkan_Dynamic_Vb(slot, _VulkanDynamicVB[slot]);
			_VulkanDynamicVB[slot] = nullptr;
			unsigned usage = DX8VertexBufferClass::USAGE_DYNAMIC;
		...
		REF_PTR_SET(VertexBuffer, _VulkanDynamicVB[slot]);
		VertexBufferOffset = _VulkanDynamicVBOffset[slot];
		return;
	}
#endif
...
#if defined(RENEGADE_VULKAN)
		if (DX8Wrapper::Vulkan_Device_Active()) {
			DX8VertexBufferClass *vulkan_vb =
				static_cast<DX8VertexBufferClass *>(DynamicVBAccess->VertexBuffer);
			WWASSERT(vulkan_vb != nullptr);
			Vertices = reinterpret_cast<VertexFormatXYZNDUV2 *>(ww3d_vulkan::VB_Lock(
				vulkan_vb,
				DynamicVBAccess->VertexBufferOffset *
					vulkan_vb->FVF_Info().Get_FVF_Size(),
				DynamicVBAccess->Get_Vertex_Count() *
					DynamicVBAccess->VertexBuffer->FVF_Info().Get_FVF_Size()));
		} else
#endif
...
#if defined(RENEGADE_VULKAN)
		if (DX8Wrapper::Vulkan_Device_Active()) {
			ww3d_vulkan::VB_Unlock(static_cast<DX8VertexBufferClass *>(DynamicVBAccess->VertexBuffer));
		} else
#endif
		{
			DX8_Assert();
...
#if defined(RENEGADE_VULKAN)
void DynamicVBAccessClass::_Reset_Vulkan_Frame_Slot(uint32_t slot)
{
	if (slot >= ww3d_vulkan::kMaxFramesInFlight) {
		return;
	}
	Release_Retired_Vulkan_Dynamic_Vbs(slot);
	_VulkanDynamicVBOffset[slot] = 0;
}
#endif
