...
bool MeshGeometryClass::intersect_obbox_brute_force_world
(
	OBBoxIntersectionTestClass & worldtest,
	const Matrix3D & mesh_to_world
)
{
	TriClass tri;
	Vector3 wv[3];
	...
	for ( int srtri = 0; srtri < Get_Polygon_Count (); srtri++ ) {
		Matrix3D::Transform_Vector ( mesh_to_world, loc[polyverts[srtri][0]], &wv[0] );
		Matrix3D::Transform_Vector ( mesh_to_world, loc[polyverts[srtri][1]], &wv[1] );
		Matrix3D::Transform_Vector ( mesh_to_world, loc[polyverts[srtri][2]], &wv[2] );
		...
		if ( CollisionMath::Intersection_Test ( worldtest.Box, tri ) ) {
			return true;
		}
	}
	return false;
...
bool MeshGeometryClass::cast_obbox_brute_force_world
(
	OBBoxCollisionTestClass & worldtest,
	const Matrix3D & mesh_to_world
)
{
	TriClass tri;
	...
	for ( int srtri = 0; srtri < Get_Polygon_Count (); srtri++ ) {
		...
		if ( CollisionMath::Collide ( worldtest.Box, worldtest.Move, tri, Vector3 ( 0, 0, 0 ), worldtest.Result ) ) {
			polyhit = srtri;
		}

		if ( worldtest.Result->StartBad ) {
			return true;
		}
	}

	if ( polyhit != -1 ) {
		worldtest.Result->SurfaceType = Get_Poly_Surface_Type ( polyhit );
		return true;
	}
	return false;
}
...
WW3DErrorType MeshGeometryClass::Load_W3D(ChunkLoadClass & cload)
{
	...
	*/
	...
	/*
	** Collision meshes need a culling tree.  Embedded W3D trees can disagree with
	** the loaded triangle list (tree traversal misses while brute-force hits).
	*/
