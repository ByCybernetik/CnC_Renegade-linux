...
static bool assetmgr_try_load_w3d_variants( const char * filename )
{
	if ( assetmgr_try_load_w3d_file( filename ) ) {
		return true;
	}

	{
		StringClass parent_file( "../", true );
		parent_file += filename;
		if ( assetmgr_try_load_w3d_file( parent_file ) ) {
			return true;
		}
	}
...
static void assetmgr_suffix_to_w3d_filename( const char * suffix, char * out, int out_size );

static bool assetmgr_ensure_htree_loaded( const char * hierarchy_name )
{
	HTreeClass *tree;
	WW3DAssetManager *mgr;
	char filename[ MAX_PATH ];

	if ( hierarchy_name == NULL || hierarchy_name[ 0 ] == '\0' ) {
		return false;
	}

	mgr = WW3DAssetManager::Get_Instance();
	if ( mgr == NULL ) {
		return false;
	}

	tree = mgr->Peek_HTree( hierarchy_name );
	if ( tree != NULL ) {
		return true;
	}

	assetmgr_suffix_to_w3d_filename( hierarchy_name, filename, (int)sizeof( filename ) );
	if ( filename[ 0 ] != '\0' ) {
		assetmgr_try_load_w3d_variants( filename );
	}

	return mgr->Peek_HTree( hierarchy_name ) != NULL;
}
...
static void assetmgr_suffix_to_w3d_filename( const char * suffix, char * out, int out_size )
{
	...
	if ( suffix == NULL || out == NULL || out_size < 6 ) {
	...
	strncpy( out, suffix, (size_t)( out_size - 5 ) );
	out[ out_size - 5 ] = '\0';
	for ( i = 0; out[ i ] != '\0'; i++ ) {
 ...
 *   12/21/97   GTH : Created.                                                                 *
 *=============================================================================================*/
void WW3DAssetManager::load_nested_assets( ChunkLoadClass & cload )
{
	while ( cload.Open_Chunk() ) {
		...
		switch ( chunk_id ) {
			...
			default:
				if ( Find_Prototype_Loader( chunk_id ) != NULL ) {
					Load_Prototype( cload );
				} else {
					load_nested_assets( cload );
				}
				break;
		}
...
HAnimClass * WW3DAssetManager::find_loaded_hanim( const char * name )
{
	const char *dot;
	const char *suffix;
	HAnimClass *anim;
	HAnimManagerIterator it( HAnimManager );
	...
	for ( it.First(); !it.Is_Done(); it.Next() ) {
		HAnimClass *candidate = it.Get_Current_Anim();
		...
		if ( stricmp( candidate_name, name ) == 0 ) {
			candidate->Add_Ref();
			return candidate;
		}

		candidate_dot = strchr( candidate_name, '.' );
		if ( candidate_dot != NULL && stricmp( candidate_dot + 1, suffix ) == 0 ) {
			candidate->Add_Ref();
			return candidate;
		}
	}
...
RenderObjClass * WW3DAssetManager::Create_Render_Obj(const char * name)
{
	...
	if (WW3D_Load_On_Demand && proto == NULL) {	// If we didn't find one, try to load on demand
		...
		if ( !assetmgr_try_load_w3d_file( filename ) ) {
			StringClass parent_file( "../", true );
			parent_file += filename;
			assetmgr_try_load_w3d_file( parent_file );
			parent_file = StringClass( "..\\", true );
			parent_file += filename;
			assetmgr_try_load_w3d_file( parent_file );
		}
...
{
	char name_buf[ 2 * W3D_NAME_LEN + 2 ];
	...
	HAnimClass * anim = HAnimManager.Get_Anim( lookup_name );

	if (WW3D_Load_On_Demand && anim == NULL) {	// If we didn't find it, try to load on demand
		
		if ( !HAnimManager.Is_Missing( lookup_name ) ) {	// if this is NOT a known missing anim

			HAnimManager.Unregister_Missing( lookup_name );

			AssetStatusClass::Peek_Instance()->Report_Load_On_Demand_HAnim( lookup_name );

			char filename[ MAX_PATH ];
			char suffix_file[ MAX_PATH ];
			const char *dot = strchr( lookup_name, '.' );
			int load_ok = 0;

			suffix_file[ 0 ] = '\0';
			if ( dot != NULL ) {
				int prefix_len = (int)( dot - lookup_name );
				if ( prefix_len <= 0 || prefix_len >= (int)sizeof( filename ) ) {
					return NULL;
				}
				memcpy( filename, lookup_name, (size_t)prefix_len );
			...
			/* Combined W3D (e.g. IF_LVL00LOAD.w3d for backdrop anims). */
			if ( assetmgr_try_load_w3d_variants( filename ) ) {
				load_ok = 1;
			}

			anim = HAnimManager.Get_Anim( lookup_name );

			/* First-person weapon anims live in per-state files (f_ga_rifl_idle.w3d). */
			if ( anim == NULL && dot[ 1 ] != '\0' ) {
				assetmgr_suffix_to_w3d_filename( dot + 1, suffix_file, (int)sizeof( suffix_file ) );
				if ( suffix_file[ 0 ] != '\0' && assetmgr_try_load_w3d_variants( suffix_file ) ) {
					load_ok = 1;
				}
				anim = HAnimManager.Get_Anim( lookup_name );
			}
			...
			if ( anim == NULL ) {
				anim = find_loaded_hanim( lookup_name );
			}

			if ( anim != NULL ) {
				HAnimManager.Unregister_Missing( lookup_name );
			} else {
				HAnimManager.Register_Missing( lookup_name );		// This is now a KNOWN missing anim
				AssetStatusClass::Peek_Instance()->Report_Missing_HAnim( lookup_name );
			}
		}
	}
...
HTreeClass *	WW3DAssetManager::Get_HTree(const char * name)
{
	...
	if (WW3D_Load_On_Demand && htree == NULL) {	// If we didn't find it, try to load on demand
		...
		char filename[ MAX_PATH ];
		assetmgr_suffix_to_w3d_filename( name, filename, (int)sizeof( filename ) );
		if ( filename[ 0 ] != '\0' ) {
			assetmgr_try_load_w3d_variants( filename );
		}
...
HAnimClass * WW3DAssetManager::Peek_HAnim(const char * name)
{
	return HAnimManager.Peek_Anim( name );
}


bool WW3DAssetManager::Ensure_HTree_Loaded( const char * hierarchy_name )
{
	return assetmgr_ensure_htree_loaded( hierarchy_name );
}


bool WW3DAssetManager::Ensure_W3D_File_Loaded( const char * asset_basename )
{
	char filename[ MAX_PATH ];

	if ( asset_basename == NULL || asset_basename[ 0 ] == '\0' ) {
		return false;
	}

	assetmgr_suffix_to_w3d_filename( asset_basename, filename, (int)sizeof( filename ) );
	if ( filename[ 0 ] == '\0' ) {
		return false;
	}

	return assetmgr_try_load_w3d_variants( filename );
}


void WW3DAssetManager::Clear_HAnim_Missing( const char * name )
{
	if ( name != NULL && name[ 0 ] != '\0' ) {
		HAnimManager.Unregister_Missing( name );
	}
}


HAnimClass * WW3DAssetManager::Resolve_HAnim( const char * name )
{
	return find_loaded_hanim( name );
}

HTreeClass * WW3DAssetManager::Peek_HTree(const char * name)
{
	return HTreeManager.Get_Tree( name );
}

bool WW3DAssetManager::Is_HAnim_Missing(const char * name)
{
	return HAnimManager.Is_Missing( name );
}
