...
int HRawAnimClass::Load_W3D(ChunkLoadClass & cload)
{
	...
	MotionChannelClass * newchan = NULL;
	BitChannelClass * newbitchan = NULL;
	W3dAnimHeaderStruct aheader;
	HTreeClass * base_pose = NULL;
   ...
   strncpy(HierarchyName,aheader.HierarchyName,W3D_NAME_LEN);
	...
	/*
	** Use Peek_HTree during Load_3D_Assets so we do not re-enter Load_3D_Assets from Get_HTree
	** while the hierarchy chunk from the same file is being parsed.
	*/
	if ( !WW3DAssetManager::Get_Instance()->Ensure_HTree_Loaded( HierarchyName ) ) {
		Free();
		return LOAD_ERROR;
	}
	base_pose = WW3DAssetManager::Get_Instance()->Peek_HTree( HierarchyName );
	if (base_pose == NULL) {
		Free();
		return LOAD_ERROR;
	}
