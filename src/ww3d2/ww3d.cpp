...
#if defined(RENEGADE_LINUX)
	fprintf(stderr, "Renegade: DX8Wrapper::Init done\n");
	fflush(stderr);
#endif
...
#if defined(RENEGADE_LINUX)
	fprintf(stderr, "Renegade: WW3D::Init completed\n");
	fflush(stderr);
#endif
...
//	TextureClass::_Reset_Time_Stamp();
	...
	if (DX8Wrapper::Vulkan_Device_Active()) {
		ww3d_vulkan::VulkanRenderDevice::Get().Begin_Render(
			clear, clearz, color.X, color.Y, color.Z);
		return WW3D_ERROR_OK;
	}
#endif
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		WWPROFILE("VulkanRenderDevice::End_Render");
		ww3d_vulkan::VulkanRenderDevice::Get().End_Render(flip_frame);
	} else
#endif
	{
...
void WW3D::Make_Screen_Shot( const char * filename_base )
{
	...
	RenegadeD3DLockedRect lrect;

	DX8_ErrorCode(fb->LockRect(Renegade_AsD3DLockedRect(&lrect),&bounds,D3DLOCK_READONLY));

	unsigned int x,y,index,index2,width,height;
	unsigned char *const lrect_bits = (unsigned char *)Renegade_D3DLockedRect_PBits(lrect);
	...
	for (y=0; y<height; y++)
	{
		for (x=0; x<width; x++)
		{
			...
			image[index]=*(lrect_bits + index2+2);
			image[index+1]=*(lrect_bits + index2+1);
			image[index+2]=*(lrect_bits + index2+0);
		}
	}
...
void WW3D::Update_Movie_Capture( void )
{
	...
	for (y=0; y<height; y++)
	{
		for (x=0; x<width; x++)
		{
			...
			image[index]=*(lrect_bits + index2+0);
			image[index+1]=*(lrect_bits + index2+1);
			image[index+2]=*(lrect_bits + index2+2);
		}
	}
