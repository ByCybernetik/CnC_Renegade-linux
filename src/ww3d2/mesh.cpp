...
{
	if ( !LINUX_MESH_CAST_GATE ( *this, raytest.CollisionType ) ) {
		return false;
	}	
...
{
	if ( !LINUX_MESH_CAST_GATE ( *this, boxtest.CollisionType ) ) {
		return false;
	}
...
#if defined(RENEGADE_COLLISION_FIX)
	/*
	** World-space triangle cast avoids bad local transforms from HLod bones.
	*/
	if ( Model->cast_obbox_brute_force_world ( boxtest, Get_Transform () ) ) {
		boxtest.CollidedRenderObj = this;
		return true;
	}
	return false;
...
{
	if ( !LINUX_MESH_INTERSECT_GATE ( *this, boxtest.CollisionType ) ) {
		return false;
	}
...
#if defined(RENEGADE_COLLISION_FIX)
	return Model->intersect_obbox_brute_force_world ( boxtest, Get_Transform () );
#else
	Matrix3D inv_tm;
	Get_Transform().Get_Orthogonal_Inverse(inv_tm);
	OBBoxIntersectionTestClass local_test(boxtest,inv_tm);
