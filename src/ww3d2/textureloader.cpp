...
IDirect3DTexture8* TextureLoader::Load_Thumbnail(const StringClass& filename)//,WW3DFormat texture_format)
{
	...
	unsigned level=0;
	RenegadeD3DLockedRect locked_rects[12];
	...
	for (level=0;level<sysmem_texture->GetLevelCount();++level) {
		DX8_ErrorCode(
			sysmem_texture->LockRect(
				level,
				Renegade_AsD3DLockedRect(&locked_rects[level]),
				NULL,
				0));
	}
	...
	for (level=0;level<sysmem_texture->GetLevelCount()-1;++level) {
		BitmapHandlerClass::Copy_Image_Generate_Mipmap(
			width,
			height,
			(unsigned char*)Renegade_D3DLockedRect_PBits(locked_rects[level]),
			locked_rects[level].Pitch,
			dest_format,
			src_surface,
			src_pitch,
			src_format,
			(unsigned char*)Renegade_D3DLockedRect_PBits(locked_rects[level+1]),	// mipmap
			locked_rects[level+1].Pitch);// mipmap

		src_format=dest_format;
		src_surface=(unsigned char*)Renegade_D3DLockedRect_PBits(locked_rects[level]);
	...
	bool allow_compression)
{
	...
	DX8_ErrorCode(
		d3d_surface->LockRect(
			Renegade_AsD3DLockedRect(&locked_rect),
			NULL,
			0));

	BitmapHandlerClass::Copy_Image(
		(unsigned char*)Renegade_D3DLockedRect_PBits(locked_rect),
		width,
		height,
		locked_rect.Pitch,
		dest_format,
...
void TextureLoader::Load_Thumbnail(TextureClass *tc)
{
	...
	if (DX8Wrapper::Vulkan_Device_Active()) {
		ww3d_vulkan::Apply_Loaded_Texture(tc, false);
		return;
	}
#endif
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		if (!ww3d_vulkan::Apply_Loaded_Texture(Texture, true)) {
			ww3d_vulkan::Apply_Missing_Texture(Texture);
		}
		State = STATE_LOAD_COMPLETE;
		return;
	}
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		if (!ww3d_vulkan::Apply_Loaded_Texture(Texture, initialize)) {
			ww3d_vulkan::Apply_Missing_Texture(Texture);
		}
		return;
	}
	...
	for (unsigned int i = 0; i < MipLevelCount; ++i) {
		...
		DX8_ErrorCode(
			D3DTexture->LockRect(
				...
				NULL,
				0));
		LockedSurfacePtr[i]		= (unsigned char *)Renegade_D3DLockedRect_PBits(locked_rect);
