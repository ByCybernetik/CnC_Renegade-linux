	...
	SurfaceFormat(format)
{
	...
	if (DX8Wrapper::Vulkan_Device_Active()) {
		ww3d_vulkan::VkCpuSurface *cpu = new ww3d_vulkan::VkCpuSurface();
		if (cpu != nullptr && cpu->Create(width, height, format)) {
			Vulkan_Cpu_Surface = cpu;
			SurfaceFormat = format;
			return;
		}
		delete cpu;
	...
	SurfaceFormat(WW3D_FORMAT_UNKNOWN)
{
	...
	if (DX8Wrapper::Vulkan_Device_Active()) {
		ww3d_vulkan::VkCpuSurface *cpu = new ww3d_vulkan::VkCpuSurface();
		if (cpu != nullptr && cpu->Load_From_File(filename)) {
			Vulkan_Cpu_Surface = cpu;
...
{
#if defined(RENEGADE_VULKAN)
	if (Vulkan_Cpu_Surface != nullptr) {
		delete static_cast<ww3d_vulkan::VkCpuSurface *>(Vulkan_Cpu_Surface);
		Vulkan_Cpu_Surface = nullptr;
	}
#endif
	if (D3DSurface) {
		D3DSurface->Release();
...
#if defined(RENEGADE_VULKAN)
	if (Vulkan_Cpu_Surface != nullptr) {
		ww3d_vulkan::VkCpuSurface *cpu =
			static_cast<ww3d_vulkan::VkCpuSurface *>(Vulkan_Cpu_Surface);
		cpu->Get_Description(&surface_desc.Width, &surface_desc.Height, &surface_desc.Format);
		return;
	}
#endif
...
#if defined(RENEGADE_VULKAN)
	if (Vulkan_Cpu_Surface != nullptr) {
		return static_cast<ww3d_vulkan::VkCpuSurface *>(Vulkan_Cpu_Surface)->Lock(pitch);
	}
#endif
	RenegadeD3DLockedRect lock_rect;	
	::ZeroMemory(&lock_rect, sizeof(lock_rect));
	DX8_ErrorCode(D3DSurface->LockRect(Renegade_AsD3DLockedRect(&lock_rect), 0, 0));
	*pitch = lock_rect.Pitch;
	return (void *)Renegade_D3DLockedRect_PBits(lock_rect);
}
...
#if defined(RENEGADE_VULKAN)
	if (Vulkan_Cpu_Surface != nullptr) {
		static_cast<ww3d_vulkan::VkCpuSurface *>(Vulkan_Cpu_Surface)->Unlock();
		return;
	}
#endif
...
void SurfaceClass::Clear()
{
#if defined(RENEGADE_VULKAN)
	if (Vulkan_Cpu_Surface != nullptr) {
		static_cast<ww3d_vulkan::VkCpuSurface *>(Vulkan_Cpu_Surface)->Clear();
		return;
	}
#endif
	...
	DX8_ErrorCode(D3DSurface->LockRect(Renegade_AsD3DLockedRect(&lock_rect),0,0));
	unsigned int i;
	unsigned char *mem=(unsigned char *) Renegade_D3DLockedRect_PBits(lock_rect);
...
void SurfaceClass::Copy(Vector2i &min,Vector2i &max, const unsigned char *other)
{
	...
	rect.top=min.J;
	rect.bottom=max.J;
	DX8_ErrorCode(D3DSurface->LockRect(Renegade_AsD3DLockedRect(&lock_rect),&rect,0));
	int i;
	unsigned char *mem=(unsigned char *) Renegade_D3DLockedRect_PBits(lock_rect);	
	int dx=max.I-min.I;
...
unsigned char *SurfaceClass::CreateCopy(int *width,int *height,int*size,bool flip)
{
	...
	*size=mysize;
	...
	RenegadeD3DLockedRect lock_rect;	
	::ZeroMemory(&lock_rect, sizeof(lock_rect));
	DX8_ErrorCode(D3DSurface->LockRect(Renegade_AsD3DLockedRect(&lock_rect),0,D3DLOCK_READONLY));
	unsigned int i;
	unsigned char *mem=(unsigned char *) Renegade_D3DLockedRect_PBits(lock_rect);
...
#if defined(RENEGADE_VULKAN)
	if (Vulkan_Cpu_Surface != nullptr && other->Vulkan_Cpu_Surface != nullptr) {
		static_cast<ww3d_vulkan::VkCpuSurface *>(Vulkan_Cpu_Surface)->Copy(
			dstx,
			dsty,
			srcx,
			srcy,
			width,
			height,
			static_cast<const ww3d_vulkan::VkCpuSurface *>(other->Vulkan_Cpu_Surface));
		return;
	}
#endif
...
void SurfaceClass::FindBB(Vector2i *min,Vector2i*max)
{
#if defined(RENEGADE_VULKAN)
	if (Vulkan_Cpu_Surface != nullptr) {
		static_cast<ww3d_vulkan::VkCpuSurface *>(Vulkan_Cpu_Surface)->Find_BB(min, max);
		return;
	}
#endif
	SurfaceDescription sd;
	...
	DX8_ErrorCode(D3DSurface->LockRect(Renegade_AsD3DLockedRect(&lock_rect),&rect,D3DLOCK_READONLY));
	...
	for (y = min->J; y < max->J; y++) {
		for (x = min->I; x < max->I; x++) {
			...
			unsigned char *base = (unsigned char *)Renegade_D3DLockedRect_PBits(lock_rect);
			unsigned char *alpha = base + (y - min->J) * lock_rect.Pitch + (x - min->I) * size;
...
bool SurfaceClass::Is_Transparent_Column(unsigned int column)
{
#if defined(RENEGADE_VULKAN)
	if (Vulkan_Cpu_Surface != nullptr) {
		return static_cast<const ww3d_vulkan::VkCpuSurface *>(Vulkan_Cpu_Surface)
			->Is_Transparent_Column(column);
	}
#endif
	SurfaceDescription sd;
	Get_Description(sd);
	...
	for (y = 0; y < (int) sd.Height; y++)
	{
		// HY - this is not endian safe
		unsigned char *base = (unsigned char *)Renegade_D3DLockedRect_PBits(lock_rect);
		unsigned char *alpha = base + y * lock_rect.Pitch;		
		unsigned char myalpha=alpha[size-1];		
		myalpha=(myalpha>>(8-alphabits)) & mask;		
		if (myalpha) {
			DX8_ErrorCode(D3DSurface->UnlockRect());
...
void SurfaceClass::Get_Pixel(Vector3 &rgb, int x,int y)
{
	...
	DX8_ErrorCode(D3DSurface->LockRect(Renegade_AsD3DLockedRect(&lock_rect),&rect,D3DLOCK_READONLY));	
	Convert_Pixel(rgb,sd,(unsigned char *) Renegade_D3DLockedRect_PBits(lock_rect));
	DX8_ErrorCode(D3DSurface->UnlockRect());	
}
...
void SurfaceClass::DrawPixel(const unsigned int x,const unsigned int y, unsigned int color)
{
	...
	DX8_ErrorCode(D3DSurface->LockRect(Renegade_AsD3DLockedRect(&lock_rect),&rect,0));
	unsigned char *cptr=(unsigned char*)Renegade_D3DLockedRect_PBits(lock_rect);
	unsigned short *sptr=(unsigned short*)Renegade_D3DLockedRect_PBits(lock_rect);
	unsigned int *lptr=(unsigned int*)Renegade_D3DLockedRect_PBits(lock_rect);
