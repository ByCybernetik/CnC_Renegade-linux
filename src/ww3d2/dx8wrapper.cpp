...
{
	DX8Wrapper::Notify_Window_Resized(width, height);
}
#endif
...
Matrix4								DX8Wrapper::TextureMatrices[MAX_TEXTURE_STAGES];
...
#if defined(RENEGADE_VULKAN)
float								DX8Wrapper::FogStart										= 0.0f;
float								DX8Wrapper::FogEnd										= 1.0f;
#endif
...
#if defined(RENEGADE_VULKAN)
void DX8Wrapper::Enumerate_Vulkan_Devices(void)
{
	RenderDeviceDescClass desc;
	desc.set_device_name("Vulkan");
	desc.set_driver_name("ww3d2_vulkan");
	desc.set_driver_version("1.0");
	desc.add_resolution(DEFAULT_RESOLUTION_WIDTH, DEFAULT_RESOLUTION_HEIGHT, DEFAULT_BIT_DEPTH);
	desc.add_resolution(1024, 768, 32);

	StringClass device_name("Vulkan", true);
...
bool DX8Wrapper::Init(void * hwnd, bool lite)
{
	...
	*/
	...
	if (!lite) {
#if defined(RENEGADE_VULKAN)
		if (ww3d_vulkan::Is_Enabled()) {
			DX8Wrapper::Enumerate_Vulkan_Devices();
			IsInitted = true;
			WWDEBUG_SAY(("DX8Wrapper Init completed (Vulkan backend)\n"));
			return true;
		}
#endif
#if !(defined(RENEGADE_LINUX) && defined(RENEGADE_VULKAN))
#if defined(RENEGADE_LINUX)
		...
		if (Direct3DCreate8Ptr) {
			...
			*/
			...
			fprintf(stderr, "Renegade: Direct3DCreate8...\n");
			fflush(stderr);
#endif
...
#if defined(RENEGADE_LINUX)
			fprintf(stderr, "Renegade: Enumerate_Devices...\n");
			fflush(stderr);
			Platform_Pump_Events();
#endif
			Enumerate_Devices();
#if defined(RENEGADE_LINUX)
			fprintf(stderr, "Renegade: Enumerate_Devices done\n");
			fflush(stderr);
			Platform_Pump_Events();
#endif
			WWDEBUG_SAY(("DX8Wrapper Init completed\n"));
		}
#endif /* !(RENEGADE_LINUX && RENEGADE_VULKAN) */
	}
	return(true);
}
...
{
#define RENEGADE_DX8_INIT_STEP(NAME) \
	do { \
		if (getenv("RENEGADE_DX8_INIT_TRACE")) { \
			fprintf(stderr, "Renegade DX8 init: %s\n", NAME); \
			fflush(stderr); \
		} \
	} while (0)
	/*
	...
	*/
	RENEGADE_DX8_INIT_STEP("Compute_Caps");
	Compute_Caps(D3DFormat_To_WW3DFormat(_PresentParameters.BackBufferFormat));

   /*
	** Initalize any other subsystems inside of WW3D
	*/
	RENEGADE_DX8_INIT_STEP("MissingTexture::_Init");
	MissingTexture::_Init();
	RENEGADE_DX8_INIT_STEP("TextureClass::_Init_Filters");
	TextureClass::_Init_Filters((TextureClass::TextureFilterMode)WW3D::Get_Texture_Filter());
	RENEGADE_DX8_INIT_STEP("TheDX8MeshRenderer.Init");
	TheDX8MeshRenderer.Init();
	RENEGADE_DX8_INIT_STEP("BoxRenderObjClass::Init");
	BoxRenderObjClass::Init();
	RENEGADE_DX8_INIT_STEP("VertexMaterialClass::Init");
	VertexMaterialClass::Init();
	RENEGADE_DX8_INIT_STEP("PointGroupClass::_Init");
	PointGroupClass::_Init(); // This needs the VertexMaterialClass to be initted
#if !defined(RENEGADE_LINUX)
	RENEGADE_DX8_INIT_STEP("ShatterSystem::Init");
	ShatterSystem::Init();
#else
	if (getenv("RENEGADE_DX8_INIT_TRACE")) {
		fprintf(stderr, "Renegade DX8 init: ShatterSystem::Init skipped on Linux\n");
		fflush(stderr);
	}
#endif
	RENEGADE_DX8_INIT_STEP("TextureLoader::Init");
	TextureLoader::Init();

	RENEGADE_DX8_INIT_STEP("Set_Default_Global_Render_States");
	Set_Default_Global_Render_States();
#undef RENEGADE_DX8_INIT_STEP
}
...
{
	for (int a=0;a<(int)(sizeof(RenderStates)/sizeof(unsigned));++a) {
...
bool DX8Wrapper::Create_Device(void)
{
	...
	if (ww3d_vulkan::Is_Enabled()) {
		...
		strncpy(
			CurrentAdapterIdentifier.Description,
			"Vulkan",
			sizeof(CurrentAdapterIdentifier.Description) - 1);
		strncpy(
			CurrentAdapterIdentifier.Driver,
			"ww3d2_vulkan",
			sizeof(CurrentAdapterIdentifier.Driver) - 1);

		fprintf(
			stderr,
			"Renegade: Vulkan Create_Device %dx%d windowed=%d\n",
			ResolutionWidth,
			ResolutionHeight,
			IsWindowed ? 1 : 0);
		fflush(stderr);

		if (!ww3d_vulkan::Try_Create_Device(
				(unsigned)ResolutionWidth,
				(unsigned)ResolutionHeight,
				IsWindowed,
				true)) {
			return false;
		}

		g_VulkanDeviceActive = true;
		ww3d_vulkan::Native_Render_State_Reset();
		for (unsigned i = 0; i < MAX_TEXTURE_STAGES; ++i) {
			TextureMatrices[i].Make_Identity();
		}
		Platform_Set_Window_Resize_Handler(Vulkan_Window_Resize_Handler);
		Do_Onetime_Device_Dependent_Inits();
		fprintf(stderr, "Renegade: Vulkan Create_Device OK\n");
		fflush(stderr);
		return true;
	}
#endif
	...
	fprintf(
		stderr,
		"Renegade: CreateDevice %dx%d windowed=%d\n",
		ResolutionWidth,
		ResolutionHeight,
		IsWindowed ? 1 : 0);
	fflush(stderr);
	...
	*/
#if defined(RENEGADE_LINUX)
	fprintf(stderr, "Renegade: Do_Onetime_Device_Dependent_Inits...\n");
	fflush(stderr);
#endif
	Do_Onetime_Device_Dependent_Inits();
#if defined(RENEGADE_LINUX)
	fprintf(stderr, "Renegade: Create_Device OK\n");
	fflush(stderr);
	Platform_Pump_Events();
#endif
	return true;
...
bool DX8Wrapper::Reset_Device(void)
{
	...
	DX8_THREAD_ASSERT();
#if defined(RENEGADE_VULKAN)
	if ((IsInitted) && g_VulkanDeviceActive) {
		...
		ww3d_vulkan::Try_Resize(
			(unsigned)ResolutionWidth,
			(unsigned)ResolutionHeight);
		DX8TextureManagerClass::Recreate_Textures();
		Invalidate_Cached_Render_States();
		Set_Default_Global_Render_States();
		WWDEBUG_SAY(("Vulkan device reset completed\n"));
		return true;
	}
#endif
...
#if defined(RENEGADE_VULKAN)
	if (g_VulkanDeviceActive) {
		Platform_Set_Window_Resize_Handler(NULL);
		ww3d_vulkan::Try_Release_Device();
		g_VulkanDeviceActive = false;
...
#else
				(int)((id.DriverVersionHighPart >> 16) & 0xFFFF),
				(int)(id.DriverVersionHighPart & 0xFFFF),
				(int)((id.DriverVersionLowPart >> 16) & 0xFFFF),
				(int)(id.DriverVersionLowPart & 0xFFFF)
#endif
				);
			...
			*/
			{
				...
				if (SUCCEEDED(D3DInterface->GetAdapterDisplayMode(adapter_index, &desktop))) {
					int bits = 32;
					...
					if (desktop.Width > 0 && desktop.Height > 0) {
						desc.add_resolution(desktop.Width, desktop.Height, bits);
					}
				}
				desc.add_resolution(DEFAULT_RESOLUTION_WIDTH, DEFAULT_RESOLUTION_HEIGHT, DEFAULT_BIT_DEPTH);
				desc.add_resolution(1024, 768, 32);
...
{
#if defined(RENEGADE_LINUX)
	// Wayland/SDL3 WSI: prefer windowed first on Linux.
...
#if defined(RENEGADE_VULKAN)
void DX8Wrapper::Record_Texture_Stage_State(
	unsigned stage,
	unsigned tss,
	unsigned value)
{
	...
	ww3d_vulkan::Native_Render_State_On_Tss(stage, tss, value);
	if (tss == D3DTSS_TEXTURETRANSFORMFLAGS || tss == D3DTSS_TEXCOORDINDEX) {
		Record_Texture_Matrix(stage);
	}
}

void DX8Wrapper::Record_Texture_Matrix(unsigned stage)
{
	if (!Vulkan_Device_Active() || stage >= MAX_TEXTURE_STAGES) {
		return;
	}
	ww3d_vulkan::Native_Render_State_Sync_Stage_Matrix(
		stage,
		TextureMatrices[stage],
		TextureStageStates[stage][D3DTSS_TEXTURETRANSFORMFLAGS]);
}

#if defined(RENEGADE_VULKAN)
void DX8Wrapper::Vulkan_Notify_Frame_End(bool present, bool present_ok)
{
	if (present && present_ok) {
		IsDeviceLost = false;
		FrameCount++;
	}
}

void DX8Wrapper::Vulkan_Reset_Render_State_After_Frame()
{
	Set_Vertex_Buffer(NULL);
	Set_Index_Buffer(NULL, 0);
	for (unsigned i = 0; i < MAX_TEXTURE_STAGES; ++i) {
...
void DX8Wrapper::Vulkan_Draw_Triangles(
	unsigned short start_index,
	unsigned short polygon_count,
	unsigned short min_vertex_index,
	unsigned short vertex_count)
{
	ww3d_vulkan::VulkanRenderDevice::Get().Draw_Triangles(
		start_index,
		polygon_count,
		min_vertex_index,
		vertex_count);
...
void DX8Wrapper::Vulkan_Draw_Strip(
	unsigned short start_index,
	unsigned short polygon_count,
	unsigned short min_vertex_index,
	unsigned short vertex_count)
{
	ww3d_vulkan::VulkanRenderDevice::Get().Draw_Strip(
		start_index,
		polygon_count,
		min_vertex_index,
		vertex_count);
...
void DX8Wrapper::Vulkan_Bind_Texture_Stages(TextureClass *const *textures, unsigned count)
{
	if (textures == nullptr) {
		return;
	}
	if (count > MAX_TEXTURE_STAGES) {
		count = MAX_TEXTURE_STAGES;
	}
...
void DX8Wrapper::Notify_Window_Resized(int width, int height)
{
	if (width <= 0 || height <= 0) {
		return;
	}
	if (g_VulkanDeviceActive) {
		if (width == ResolutionWidth && height == ResolutionHeight) {
			ww3d_vulkan::Try_Resize((unsigned)width, (unsigned)height);
			return;
		}
...
#if defined(RENEGADE_VULKAN)
	if (Vulkan_Device_Active()) {
		ww3d_vulkan::VulkanRenderDevice::Get().Get_Render_Target_Resolution(
			set_w,
			set_h);
		set_bits = BitDepth;
		set_windowed = IsWindowed;
...
#if defined(RENEGADE_LINUX)
		// Win32 registry stubs are inert; render device is already configured.
		return true;
#else
		return false;
#endif
...
#if defined(RENEGADE_LINUX)
	Platform_Pump_Events();
	fprintf(stderr, "Renegade: Registry_Load fast path Set_Render_Device\n");
	fflush(stderr);
	if (_RenderDeviceNameTable.Count() > 0) {
		if (Set_Render_Device(
				0,
				...
				1,
				false)) {
			fprintf(stderr, "Renegade: Set_Render_Device OK\n");
			fflush(stderr);
			return true;
		}
		fprintf(stderr, "Renegade: Set_Render_Device failed, trying Set_Any\n");
		fflush(stderr);
	}
#endif
	return Set_Any_Render_Device();
...
#if defined(RENEGADE_VULKAN)
	if (Vulkan_Device_Active()) {
		ww3d_vulkan::VulkanRenderDevice::Get().Begin_Frame();
		return;
	}
#endif
	DX8CALL(BeginScene());
...
void DX8Wrapper::End_Scene(bool flip_frames)
{
	...
	if (Vulkan_Device_Active()) {
		const bool ok =
			ww3d_vulkan::VulkanRenderDevice::Get().End_Frame(flip_frames);
		Vulkan_Notify_Frame_End(flip_frames, ok);
		Vulkan_Reset_Render_State_After_Frame();
		return;
	}
#endif
	DX8CALL(EndScene());
	...
	if (flip_frames) {
		...
		if (Get_Current_Caps()->Get_Vendor()==DX8Caps::VENDOR_ATI) {
			...
			RenegadeD3DLockedRect rect;
			HRESULT res=bb->LockRect(
					Renegade_AsD3DLockedRect(&rect),
					NULL,
					D3DLOCK_READONLY);
			if (res==D3D_OK) {
				bb->UnlockRect();
...
void DX8Wrapper::Clear(bool clear_color, bool clear_z_stencil, const Vector3 &color, float z, unsigned int stencil)
{
	...
	if (Vulkan_Device_Active()) {
		(void)z;
		(void)stencil;
		ww3d_vulkan::ClearDesc clear_desc;
		clear_desc.color = clear_color;
		clear_desc.depth = clear_z_stencil;
		clear_desc.r = color.X;
		clear_desc.g = color.Y;
		clear_desc.b = color.Z;
		clear_desc.a = 1.0f;
		ww3d_vulkan::VulkanRenderDevice::Get().Clear(clear_desc);
		return;
	}
#endif
...
void DX8Wrapper::Set_Viewport(CONST D3DVIEWPORT8* pViewport)
{
	DX8_THREAD_ASSERT();
#if defined(RENEGADE_VULKAN)
	if (Vulkan_Device_Active() && pViewport != nullptr) {
		ww3d_vulkan::ViewportDesc vp;
		vp.x = pViewport->X;
		vp.y = pViewport->Y;
		vp.w = pViewport->Width;
		vp.h = pViewport->Height;
		ww3d_vulkan::VulkanRenderDevice::Get().Set_Viewport(vp);
		return;
	}
#endif
	...
	unsigned short vertex_count)
{
	...
	if (Vulkan_Device_Active()) {
		...
		draw_desc.vertex_offset = dyn_vb_access.VertexBufferOffset;
		draw_desc.vertex_count = vertex_count;
		draw_desc.shader = &render_state.shader;
		draw_desc.vertex_buffer = vb;
		draw_desc.index_buffer = ib;
		draw_desc.fvf = dyn_vb_access.FVF_Info().Get_FVF();
		ww3d_vulkan::VulkanRenderDevice::Get().Draw_Indexed(draw_desc);
...
#if defined(RENEGADE_VULKAN)
	if (Vulkan_Device_Active()) {
		ww3d_vulkan::VulkanRenderDevice::Get().Draw_Primitive(
			primitive_type,
			start_index,
			polygon_count,
			min_vertex_index,
		...
		case BUFFER_TYPE_DYNAMIC_DX8:
			{
				...
				if (Vulkan_Device_Active()) {
					DX8VertexBufferClass *vb =
						static_cast<DX8VertexBufferClass *>(render_state.vertex_buffer);
					DX8IndexBufferClass *ib =
						static_cast<DX8IndexBufferClass *>(render_state.index_buffer);
					if (vb != nullptr && ib != nullptr) {
						/* Match D3D8 SetIndices BaseVertexIndex (index_base + vba).
						 * min_vertex_index is only a VB range hint for DrawIndexedPrimitive. */
						ww3d_vulkan::DrawIndexedDesc draw_desc;
						draw_desc.primitive_type = primitive_type;
						draw_desc.start_index =
							start_index + render_state.iba_offset;
						draw_desc.polygon_count = polygon_count;
						draw_desc.vertex_offset =
						...
						draw_desc.index_buffer = ib;
						draw_desc.fvf =
							render_state.vertex_buffer->FVF_Info().Get_FVF();
						ww3d_vulkan::VulkanRenderDevice::Get().Draw_Indexed(
							draw_desc);
					}
					break;
				}
...
void DX8Wrapper::Compute_Caps(WW3DFormat display_format)
{
	...
	if (Vulkan_Device_Active()) {
		...
		caps.RasterCaps = D3DPRASTERCAPS_FOGRANGE | D3DPRASTERCAPS_ZTEST;
...
#if defined(RENEGADE_VULKAN)
	if (Vulkan_Device_Active()) {
		return ww3d_vulkan::VulkanRenderDevice::Get().Create_Render_Target(
			width,
			height,
			format);
	}
...
#if defined(RENEGADE_VULKAN)
	if (Vulkan_Device_Active()) {
		ww3d_vulkan::VulkanRenderDevice::Get().Set_Render_Target(texture);
...
#if defined(RENEGADE_VULKAN)
		if (Vulkan_Device_Active()) {
			ww3d_vulkan::VulkanRenderDevice::Get().Restore_Default_Render_Target();
			IsRenderToTexture = false;
			return;
		}
#endif
...
static void Vector3_To_Float4(const Vector3 &v, float alpha, float out[4])
{
	out[0] = v.X;
	out[1] = v.Y;
	out[2] = v.Z;
...
static void Color_To_Float4(unsigned color, float out[4])
{
	Vector4 c = DX8Wrapper::Convert_Color(color);
	out[0] = c[0];
	out[1] = c[1];
	out[2] = c[2];
	out[3] = c[3];
...
static void D3d_Color_To_Float4(const D3DCOLORVALUE &c, float out[4])
{
	out[0] = c.r;
	out[1] = c.g;
	out[2] = c.b;
...
void Fill_Vulkan_Draw_State(VulkanDrawState *state)
{
	...
	Matrix_To_Float16(view_row, state->view);

	const VertexMaterialClass *material = rs.material;
	const ShaderClass &shader = rs.shader;
	...
	if (material != nullptr) {
		...
		Vector3_To_Float4(ambient, 1.0f, state->material_ambient);
		Vector3_To_Float4(diffuse, 1.0f, state->material_diffuse);
		Vector3_To_Float4(emissive, 1.0f, state->material_emissive);
		...
		state->material_diffuse[0] = state->material_diffuse[1] = state->material_diffuse[2] = 1.0f;
		state->material_diffuse[3] = 1.0f;
		state->material_specular[0] = state->material_specular[1] = state->material_specular[2] = 0.0f;
		state->material_specular[3] = 1.0f;
		state->material_shininess = 1.0f;
	}
	...
	if (DX8Wrapper::Get_Fog_Enable()) {
		...
		case ShaderClass::FOG_ENABLE:
			state->fog_mode = FrameUBO::FOG_MODE_BLEND;
			fog_active = true;
			Color_To_Float4(DX8Wrapper::Get_Fog_Color(), state->fog_color);
			break;
		case ShaderClass::FOG_SCALE_FRAGMENT:
			state->fog_mode = FrameUBO::FOG_MODE_SCALE;
			fog_active = true;
			state->fog_color[0] = state->fog_color[1] = state->fog_color[2] = 0.0f;
			state->fog_color[3] = 1.0f;
			break;
		case ShaderClass::FOG_WHITE:
			state->fog_mode = FrameUBO::FOG_MODE_WHITE;
			fog_active = true;
			state->fog_color[0] = state->fog_color[1] = state->fog_color[2] = 1.0f;
			state->fog_color[3] = 1.0f;
			break;
		default:
			break;
	...
	if (fog_active) {
		state->flags |= (uint32_t)FrameUBO::FLAG_FOG;
		state->fog_start = DX8Wrapper::Get_Fog_Start();
		state->fog_end = DX8Wrapper::Get_Fog_End();
	}
	...
	for (unsigned i = 0; i < 4; ++i) {
		...
		const D3DLIGHT8 &light = rs.Lights[i];
		D3d_Color_To_Float4(light.Diffuse, state->light_diffuse[i]);

		if (light.Type == D3DLIGHT_DIRECTIONAL) {
			state->light_params[i][0] = (float)FrameUBO::LIGHT_DIRECTIONAL;
			state->light_dir_or_pos[i][0] = light.Direction.x;
			state->light_dir_or_pos[i][1] = light.Direction.y;
			state->light_dir_or_pos[i][2] = light.Direction.z;
		} else if (light.Type == D3DLIGHT_POINT) {
			state->light_params[i][0] = (float)FrameUBO::LIGHT_POINT;
			state->light_dir_or_pos[i][0] = light.Position.x;
			state->light_dir_or_pos[i][1] = light.Position.y;
			state->light_dir_or_pos[i][2] = light.Position.z;
			state->light_params[i][1] = light.Attenuation0;
			if (state->light_params[i][1] <= 0.0f) {
				state->light_params[i][1] = 1.0f;
			}
		}
	}
}
