...
void DazzleRenderObjClass::Init_Type(const DazzleInitClass& i)
{
	...
	if (current_dazzle_layer != NULL) {
		current_dazzle_layer->Ensure_Visible_List_Capacity(type_count);
	}
}
...
void DazzleRenderObjClass::Set_Layer(DazzleLayerClass *layer)
{
	...
	layer->Ensure_Visible_List_Capacity(type_count);
			...
			case DAZZLEFACTORY_CHUNKID_VARIABLES:
				...
				while (cload.Open_Micro_Chunk()) {
					switch(cload.Cur_Micro_Chunk_ID()) {
						READ_MICRO_CHUNK_WIRE_POINTER(cload, DAZZLEFACTORY_VARIABLE_OBJPOINTER, old_obj);
...
void DazzlePersistFactoryClass::Save(ChunkSaveClass & csave,PersistClass * obj)	const
{
	...
	WRITE_MICRO_CHUNK_WIRE_POINTER(csave,DAZZLEFACTORY_VARIABLE_OBJPOINTER,robj);
...
DazzleLayerClass::DazzleLayerClass(void) :
	visible_lists(NULL),
	visible_list_capacity(0)
{
	Ensure_Visible_List_Capacity(type_count);
}

void DazzleLayerClass::Ensure_Visible_List_Capacity(unsigned required_count)
{
	if (required_count <= visible_list_capacity) {
		return;
	}

	DazzleRenderObjClass **new_lists = new DazzleRenderObjClass *[required_count];
	for (unsigned int i = 0; i < required_count; ++i) {
		new_lists[i] = NULL;
	}

	if (visible_lists != NULL) {
		for (unsigned int i = 0; i < visible_list_capacity; ++i) {
			new_lists[i] = visible_lists[i];
		}
		delete [] visible_lists;
	}

	visible_lists = new_lists;
	visible_list_capacity = required_count;
}
...
void DazzleLayerClass::Render(CameraClass* camera)
{
	...
	Ensure_Visible_List_Capacity(type_count);
	if (visible_lists == NULL || type_count == 0) {
		return;
	}
	camera->Apply();
...
{
	if (visible_lists == NULL || type >= visible_list_capacity) {
		return 0;
	}
