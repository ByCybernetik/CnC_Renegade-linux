...
bool	VehicleGameObj::Save( ChunkSaveClass & csave )
{
	...
	if ( num_seats != 0 ) {
		...
		for ( int i = 0; i < num_seats; i++ ) {
			uint32 _wire = (uint32)(uintptr_t)SeatOccupants[i];
			csave.Write( &_wire, sizeof(_wire) );
		}
		csave.End_Chunk();
	}
	...
	while (cload.Open_Chunk()) {
		switch(cload.Cur_Chunk_ID()) {
			...
			case	CHUNKID_SEAT_LIST:
			{
				...
				for ( int i = 0; i < num_seats; i++ ) {
					uint32 _wire = 0;
					cload.Read( &_wire, sizeof(_wire) );
					SeatOccupants[i] = (SoldierGameObj *)(uintptr_t)_wire;
					if ( SeatOccupants[i] != NULL ) {
...
void	VehicleGameObj::On_Post_Load( void )
{
	...
	// Level .ldd files may omit PhysDynamicSaveSystem; Init_Wheel_Effects needs vehicle phys.
	int used_phys_fallback = ( Peek_Physical_Object() == NULL ) ? 1 : 0;
...
VehiclePhysClass	*VehicleGameObj::Peek_Vehicle_Phys( void ) const
{
	...
	return phys->As_VehiclePhysClass();		// NOTE: sometimes will return NULL!
}
