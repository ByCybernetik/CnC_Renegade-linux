...
void	Debug_Message( char *format, ... )
{
	...
	ScriptCommands_Debug_Message_Va( format, arg_list );
	va_end (arg_list);
#endif
}

void	ScriptCommands_Debug_Message_Va( char *format, va_list arg_list )
{
#ifdef WWDEBUG
	SCRIPT_PTR_CHECK( format );
...
void	Control_Enable( GameObject * obj, bool enable )
{
	...
	if (smart) {
		...
		//
		// Restoring player control ends cinematic camera/freeze even if
		// Set_Camera_Host(NULL) was skipped by a cinematic script.
		//
		if ( enable && obj == (GameObject *)COMBAT_STAR && COMBAT_CAMERA != NULL ) {
			COMBAT_CAMERA->Set_Host_Model( NULL );
...
**
*/
static DynamicVectorClass<FileClass *> TextFileHandleTable;

static FileClass * Text_File_Lookup( int handle )
{
	if ( handle <= 0 || handle > TextFileHandleTable.Count() ) {
		return NULL;
	}
	return TextFileHandleTable[handle - 1];
}
...
int	Text_File_Open( const char * filename )
{
	...
	TextFileHandleTable.Add( file );
	return TextFileHandleTable.Count();
}

bool	Text_File_Get_String( int handle, char * buffer, int size )
{
	FileClass * file = Text_File_Lookup( handle );
...
void	Text_File_Close( int handle )
{
	...
	if ( file != NULL ) {
		file->Close();
		_TheFileFactory->Return_File( file );
		TextFileHandleTable[handle - 1] = NULL;
	}
}
