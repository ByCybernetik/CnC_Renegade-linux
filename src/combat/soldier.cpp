			...
			case CHUNKID_VARIABLES:
				while (cload.Open_Micro_Chunk()) {
					switch(cload.Cur_Micro_Chunk_ID()) {
						...
						READ_MICRO_CHUNK_WIRE_POINTER( cload, MICROCHUNKID_VEHICLE, Vehicle );
						...
						case MICROCHUNKID_INNATE_OBSERVER_PTR:
							{
								...
								InnateObserver = reinterpret_cast<SoldierObserverClass *>(uintptr_t( _wire_ptr_u32 ));
							}
							if ( InnateObserver != NULL ) {
...
//						READ_MICRO_CHUNK( cload, MICROCHUNKID_FLAME_TIMER, FlameTimer );		
						...
						READ_MICRO_CHUNK_WIRE_POINTER( cload, MICROCHUNKID_WEAPON_MODEL, WeaponRenderModel );
...
{
	// Level .ldd files may omit PhysDynamicSaveSystem; pointer remap then leaves PhysObj NULL.
	Ensure_Phys_Object_After_Load();

	HumanPhysClass *human_phys = NULL;
	PhysClass *phys = Peek_Physical_Object();
	if ( phys != NULL ) {
		human_phys = phys->As_HumanPhysClass();
	}
	if ( human_phys != NULL ) {
		HumanState.Init_If_Needed( human_phys );
	}
	...
	// Set_HTree in Adjust_Skeleton resets the bind pose; refresh animation after skeleton tweak.
	HumanState.Finish_Gameplay_After_Load();
...
//		HumanState.Update_Aiming( Get_Weapon_Tilt(), Get_Weapon_Turn() );
		...
		if ( Get_Weapon() && WeaponRenderModel ) {
			if ( this == COMBAT_STAR && CombatManager::Is_First_Person() ) {
				// Third-person weapon mesh is a separate scene object; hide it in FP.
				WeaponRenderModel->Set_Hidden( true );
				if ( BackWeaponRenderModel != NULL ) {
					BackWeaponRenderModel->Set_Hidden( true );
				}
			} else if ( Get_Weapon()->Get_Style() == WEAPON_HOLD_STYLE_C4 || Get_Weapon()->Get_Style() == WEAPON_HOLD_STYLE_BEACON ) {
...
{	WWPROFILE( "Soldier PostThink" );
	...
	if ( Get_Anim_Control() != NULL && Get_Anim_Control()->Peek_Model() != Peek_Model() ) {
...
//			}
	...
	if ( Get_Anim_Control() != NULL && Peek_Model() != NULL ) {
		if ( Peek_Model()->Peek_Animation() == NULL && Get_Anim_Control()->Get_Animation_Name()[0] != '\0' ) {
			HumanState.Update_Animation();
			Get_Anim_Control()->Update( 0 );
		}
	}
...
//	Debug_Say(( "Height %f, width %f\n", height, width ));
	...
	// Default definition values do not change pivot data; skip redundant HTree clone.
