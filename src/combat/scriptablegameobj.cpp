...
void	ScriptableGameObj::Copy_Settings( const ScriptableGameObjDef & definition )
{
	...
	if (CombatManager::I_Am_Server()) {
		...
		for ( int i = 0; i < definition.ScriptNameList.Count(); i++ ) {
			const char *script_name = definition.ScriptNameList[i];
...
bool	ScriptableGameObj::Save( ChunkSaveClass & csave )
{
	...
	csave.Begin_Chunk( CHUNKID_VARIABLES );
		...
		WRITE_MICRO_CHUNK_WIRE_POINTER( csave, MICROCHUNKID_REFERENCEABLE_PTR, referenceable_ptr );
			...
			case CHUNKID_VARIABLES:
				while (cload.Open_Micro_Chunk()) {
					switch(cload.Cur_Micro_Chunk_ID()) {
						...
						READ_MICRO_CHUNK_WIRE_POINTER( cload, MICROCHUNKID_REFERENCEABLE_PTR, referenceable_ptr );
#else
						...
						{
							GameObjObserverClass * ptr = NULL;
							uint32 wire_ptr = 0;
#if defined(RENEGADE_LINUX)
							cload.Read(&wire_ptr, sizeof(wire_ptr));
							ptr = reinterpret_cast<GameObjObserverClass *>(uintptr_t(wire_ptr));
#else
...
void ScriptableGameObj::Start_Observers( void )
{
	...
	const GameObjObserverList & observer_list = Get_Observers();
#if defined(__GNUC__) && defined(_WIN32)
	static int s_created_log_serial = 0;
#endif
	for( int index = 0; index < observer_list.Count(); index++ ) {
#if defined(__GNUC__) && defined(_WIN32)
		const char *obs_name = observer_list[index]->Get_Name();
		if ((s_created_log_serial++ % 40) == 0) {
			char data[96];
			snprintf(data, sizeof(data), "{\"name\":\"%.40s\",\"idx\":%d}", obs_name, index);
		}
#endif
