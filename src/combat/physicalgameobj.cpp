...
bool	PhysicalGameObj::Save( ChunkSaveClass & csave )
{
	...
	csave.Begin_Chunk( CHUNKID_VARIABLES );
		...
		WRITE_MICRO_CHUNK_WIRE_POINTER( csave, MICROCHUNKID_PHYS_OBSERVER_PTR, phys_observer_ptr );
		WRITE_MICRO_CHUNK_WIRE_POINTER( csave, MICROCHUNKID_PHYSICAL_OBJECT, PhysObj );
		...
		WRITE_MICRO_CHUNK_WIRE_POINTER( csave, MICROCHUNKID_ACTIVE_CONVERSATION, ActiveConversation );
	...
	while (cload.Open_Chunk()) {
		switch(cload.Cur_Chunk_ID()) {
			...
			case CHUNKID_VARIABLES:
				while (cload.Open_Micro_Chunk()) {
					switch(cload.Cur_Micro_Chunk_ID()) {
						READ_MICRO_CHUNK_WIRE_POINTER( cload, MICROCHUNKID_PHYS_OBSERVER_PTR, phys_observer_ptr );
						READ_MICRO_CHUNK( cload, LEGACY_MICROCHUNKID_PLAYER_TYPE, PlayerType );
						READ_MICRO_CHUNK_WIRE_POINTER( cload, MICROCHUNKID_PHYSICAL_OBJECT, PhysObj );
						...
						READ_MICRO_CHUNK_WIRE_POINTER( cload, MICROCHUNKID_ACTIVE_CONVERSATION, ActiveConversation );
...
void PhysicalGameObj::Ensure_Phys_Object_After_Load( void )
{
	if ( PhysObj == NULL ) {
		Copy_Settings( Get_Definition() );
	}
}
...
//			Debug_Say(( "Hibernate!!!\n" ));
	...
	if ( AnimControl != NULL ) {
		...
		const float anim_dtime = TimeManager::Get_Frame_Seconds();

		AnimControl->Update( anim_dtime );	// update the animation control

		// Push bone transforms to the render hierarchy before the render pass.
		RenderObjClass *render_model = Peek_Model();
		Animatable3DObjClass *anim_model = render_model != NULL
			? (Animatable3DObjClass *)render_model : NULL;
		if ( anim_model != NULL && anim_model->Get_HTree() != NULL ) {
			// Get_Bone_Transform forces hierarchy evaluation when stale (public API).
			anim_model->Get_Bone_Transform( 0 );
		}
...
void	PhysicalGameObj::Reset_Hibernating( void )
{
	...
	if (Is_Hibernating ()) {
		End_Hibernation ();
		HibernationTimer = HIBERNATION_DELAY;
	} else {
