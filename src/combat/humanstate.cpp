...
void	HumanStateClass::Init_If_Needed( HumanPhysClass * human_phys )
{
	if ( human_phys == NULL ) {
		return;
	}

	if ( HumanPhys == human_phys ) {
		return;
	}

	const uintptr_t old_phys = (uintptr_t)HumanPhys;
	REF_PTR_RELEASE( HumanPhys );
	HumanPhys = human_phys;
	HumanPhys->Add_Ref();
}

void	HumanStateClass::Finish_Gameplay_After_Load( void )
{
	StateLocked = false;

	if ( AnimControl != NULL && HumanPhys != NULL ) {
...
bool	HumanStateClass::Save( ChunkSaveClass & csave )
{
	csave.Begin_Chunk( CHUNKID_VARIABLES );
		...
		WRITE_MICRO_CHUNK_WIRE_POINTER( csave, MICROCHUNKID_PHYSOBJ,	HumanPhys );
			...
			case CHUNKID_VARIABLES:
				...
				while (cload.Open_Micro_Chunk()) {
					...
					switch(cload.Cur_Micro_Chunk_ID()) {
						...
						READ_MICRO_CHUNK_WIRE_POINTER( cload, MICROCHUNKID_PHYSOBJ, HumanPhys );
