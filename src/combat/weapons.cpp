...
static void Update_Muzzle_Fx_Subobject( RenderObjClass * robj, bool flash_on )
{
	...
	Set_Muzzle_Fx_Mesh_Hidden( robj, !flash_on );
	if ( flash_on ) {
		Maybe_Trigger_Muzzle_Smoke( robj, true );
	} else {
		Set_Muzzle_Smoke_Emitters_Hidden( robj, true, true );
	}
}
...
#else
						/*
						** Logs: empty click (14k) + reload (37k) same frame = audible noise mash.
						** Skip empty when auto-reload will run immediately after.
						*/
						if ( Definition->EmptySoundDefID != 0 && !Is_Reload_Needed() ) {
						...
						} else if ( Definition->EmptySoundDefID != 0 && Is_Reload_Needed() ) {
						}
#endif
					}
				}
...
void	WeaponClass::Update( void )
{
	...
	/*
	** Logs: Do_Firing_Effects after Force_Reload same frame stacked 14k/37k gunshot + reload.
	*/
	if ( DidFire && State != STATE_RELOAD ) {
...
const Matrix3D & WeaponClass::Get_Muzzle( int index )
{
	ArmedGameObj * owner = Get_Owner();
	if ( owner == NULL ) {
		static Matrix3D _null_owner_muzzle(1);
		return _null_owner_muzzle;
	}
	...
	if ( owner == COMBAT_STAR && CombatManager::Is_First_Person() ) {
		...
		if ( COMBAT_CAMERA != NULL ) {
			_muzzle.Obj_Look_At( COMBAT_CAMERA->Get_Transform().Get_Translation(), owner->Get_Targeting_Pos(), 0 );
		}
		return _muzzle;
	}

	return owner->Get_Muzzle( index );
}
	...
	if ( Model != NULL ) {
		if (( MuzzleA0Bone > 0 ) && (LastFlashA0 != flashA0)) {
			...
			for (int i=0; i<Model->Get_Num_Sub_Objects_On_Bone( MuzzleA0Bone ); i++) {
				...
				Update_Muzzle_Fx_Subobject( robj, flashA0 );
				robj->Release_Ref();
			}
		}
		...
		if (( MuzzleA1Bone > 0 ) && (LastFlashA1 != flashA1)) {
			...
			for (int i=0; i<Model->Get_Num_Sub_Objects_On_Bone( MuzzleA1Bone ); i++) {
				...
				Update_Muzzle_Fx_Subobject( robj, flashA1 );
				robj->Release_Ref();
			}
		}
	}
