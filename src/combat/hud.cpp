...
static int Count_Weapon_Chart_Icon_Slots( WeaponBagClass *weapon_bag )
{
	int slots = 0;
	...
	for (int column = 1; column <= 10; column++) {
		for (int i = 0; i < weapon_bag->Get_Count(); i++) {
			...
			if (weapon != NULL && (int)weapon->Get_Key_Number() == (column % 10)) {
				slots++;
			}
		}
	}
...
static	void	Weapon_Chart_Update( void )
{
	...
	if ( ForceChartRebuild ||
			(Count_Weapon_Chart_Icon_Slots(weapon_bag) != WeaponChartIcons.Count())) {
...
//				if ( weapon->Is_Switching() || weapon->Get_Total_Rounds() == 0 ) {
				...
				if (index >= WeaponChartIcons.Count() ||
						WeaponChartIcons[index] == NULL) {
					if (!retry_color_pass) {
						retry_color_pass = true;
						Build_Weapon_Chart_Icons();
						goto retry_color_pass;
					}
					Clear_Weapon_Chart_Icons();
					WeaponChartTimer = 0;
					return;
...
static	void	Weapon_Chart_Render( void )
{
	...
	if (WeaponChartBoxRenderer != NULL && WeaponChartBoxRenderer->Has_Drawable_Geometry()) {
		WeaponChartBoxRenderer->Render();
	}
	for ( int i = 0; i < WeaponChartIcons.Count(); i++) {
		Render2DClass *icon = WeaponChartIcons[i];
		if (icon != NULL && icon->Has_Drawable_Geometry()) {
			icon->Render();
		}
	}
...
//		scorestring.Format( L"%d", score );
		...
		Generate_WChar_Text_From_Number(
			score_string,
			(int)(sizeof(score_string) / sizeof(score_string[0])) - 1,
			0,
			score);
...
static	void	Info_Render( void )
{
	if (InfoRenderer != NULL) {
		if (InfoRenderer->Has_Drawable_Geometry()) {
			InfoRenderer->Render();
		}
	}
	if (InfoHealthCountRenderer != NULL) {
		InfoHealthCountRenderer->Render();
	}
	if (InfoShieldCountRenderer != NULL) {
