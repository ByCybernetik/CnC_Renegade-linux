...
static HMODULE gcc_load_script_dll_logged(const char *dll_path)
{
	return LoadLibraryA(dll_path);
}

static HMODULE gcc_load_script_dll_from_exe_dir(const char *dll_name)
{
	char path[MAX_PATH];
	char *slash;
	DWORD n;
	...
	if (n == 0 || n >= MAX_PATH) {
		return gcc_load_script_dll_logged(dll_name);
	}
	...
	slash[1] = '\0';
	if (strlen(path) + strlen(dll_name) >= MAX_PATH) {
		return gcc_load_script_dll_logged(dll_name);
	}

	const size_t exe_dir_len = strlen(path);
	strcat(path, dll_name);
	HMODULE mod = gcc_load_script_dll_logged(path);
	if (mod != NULL) {
		return mod;
	}

#if defined(RENEGADE_LINUX)
	// Meson dev layout: renegade in src/commando/, scripts.so in src/scripts/.
	path[exe_dir_len] = '\0';
	strncat(path, "../scripts/scripts.so", MAX_PATH - exe_dir_len - 1);
	mod = gcc_load_script_dll_logged(path);
	if (mod != NULL) {
		return mod;
	}
#endif
...
static bool debug_script_name_readable(const char *name)
{
...
void ScriptManager::Rebind_Script_Dll(void)
{
	Destroy_Pending();

#if defined(__GNUC__) && (defined(_WIN32) || defined(RENEGADE_LINUX))
	if (hDLL != NULL) {
		Bind_Script_Dll_Exports();
		return;
	}
#endif
...
void ScriptManager::Load_Scripts(const char* dll_filename)
{
   ...
   if (!IS_SOLOPLAY && CombatManager::I_Am_Only_Client())
	...
	if (dll_filename != NULL) {
		hDLL = gcc_load_script_dll_from_exe_dir(dll_filename);
	}
	if (hDLL == NULL) {
		hDLL = gcc_load_script_dll_from_exe_dir("scripts.dll");
	}
	if (hDLL == NULL) {
		hDLL = gcc_load_script_dll_from_exe_dir("SCRIPTS.DLL");
	}
#if defined(RENEGADE_LINUX)
	if (hDLL == NULL) {
		hDLL = gcc_load_script_dll_from_exe_dir("scripts.so");
	}
#endif
#else
...
*/
#if defined(__GNUC__) && (defined(_WIN32) || defined(RENEGADE_LINUX))
void ScriptManager::Bind_Script_Dll_Exports(void)
{
	ScriptCreateFunct = (LPFN_CREATE_SCRIPT)GetProcAddress(hDLL, LPSTR_CREATE_SCRIPT);
	if (!ScriptCreateFunct) {
		Debug_Say(("Cound not find Create_Script\n"));
...
#if defined(__GNUC__) && (defined(_WIN32) || defined(RENEGADE_LINUX))
		if (!debug_script_name_readable(script_name)) {
			return NULL;
		}
#endif
...
#if defined(RENEGADE_LINUX)
				READ_MICRO_CHUNK_WIRE_POINTER( cload, MICROCHUNKID_GAME_OBJ_OBSERVER_PTR, game_obj_observer_ptr );
				READ_MICRO_CHUNK_WIRE_POINTER( cload, MICROCHUNKID_OWNER_PTR, owner_ptr );
#else
