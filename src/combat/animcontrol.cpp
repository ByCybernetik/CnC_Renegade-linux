...
void	BlendableAnimChannelClass::Update_Model( RenderObjClass	*anim_model )
{
	...
	if ( OldChannel.Peek_Animation() && NewChannel.Peek_Animation() ) {
	...
	} else if ( OldChannel.Peek_Animation() ) {
		anim_model->Set_Animation( OldChannel.Peek_Animation(), OldChannel.Get_Frame() );
	} else {
		anim_model->Set_Animation();
	}
...
void	HumanAnimControlClass::Set_Model( RenderObjClass	*anim_model )
{
	...
	// Prefer the render-obj name (S_A_*, S_B_*) over HTree name. After Adjust_Skeleton()
	// the HTree may still be "s_a_human" on female meshes, which breaks anim remapping.
	if ( anim_model != NULL ) {
		const char * model_name = anim_model->Get_Name();
		if ( model_name != NULL && model_name[0] == 'S' && model_name[1] == '_' && model_name[2] != '\0' ) {
			Skeleton = model_name[2];
		} else {
			const HTreeClass * tree = anim_model->Get_HTree();
			if ( tree != NULL ) {
				const char * name = tree->Get_Name();
				if ( name != NULL && name[2] != '\0' ) {
					Skeleton = name[2];
				}
			}
		}
...
void	HumanAnimControlClass::Update( float dtime )
{
	...
	if ( Model != NULL ) {
		...
		// Single-channel path: delegate to BlendableAnimChannelClass::Update_Model,
		// matching SimpleAnimControlClass. The DataList aggregator mishandles
		// intra-channel blends (Old/New ordering vs Update_Model).
		if ( Channel2Ratio == 0 ) {
			Channel1.Update_Model( Model );
			return;
		}

		// Dual-channel path (torso/leg mixing)
