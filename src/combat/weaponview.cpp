...
static bool Is_Muzzle_Fx_Name( const char * name )
{
	return name != NULL && ( strstr( name, "MUZZLEFLASH" ) != NULL || strstr( name, "MZ" ) != NULL );
}

// Toggle flash meshes only; particle emitters (smoke) are handled separately.
static void Set_Muzzle_Flash_Meshes_Hidden( RenderObjClass * robj, bool hidden )
{
	if ( robj == NULL ) {
		return;
	}
	...
	for ( int i = 0; i < robj->Get_Num_Sub_Objects(); i++ ) {
		...
		if ( sub->Class_ID() != RenderObjClass::CLASSID_PARTICLEEMITTER ) {
			sub->Set_Hidden( hidden );
			if ( sub->Class_ID() == RenderObjClass::CLASSID_HLOD ) {
				Set_Muzzle_Flash_Meshes_Hidden( sub, hidden );
			}
		}
		sub->Release_Ref();
	}
...
static void Set_Muzzle_Fx_Mesh_Hidden( RenderObjClass * robj, bool hidden )
{
	if ( robj == NULL || robj->Class_ID() == RenderObjClass::CLASSID_PARTICLEEMITTER ) {
		return;
	}

	if ( robj->Class_ID() == RenderObjClass::CLASSID_HLOD ) {
		Set_Muzzle_Flash_Meshes_Hidden( robj, hidden );
	} else {
		robj->Set_Hidden( hidden );
	}
}

static void Set_Muzzle_Smoke_Emitters_Hidden( RenderObjClass * robj, bool under_muzzle, bool hidden )
{
	if ( robj == NULL ) {
		return;
	}
	...
	bool muzzle = under_muzzle || Is_Muzzle_Fx_Name( name );

	for ( int i = 0; i < robj->Get_Num_Sub_Objects(); i++ ) {
		RenderObjClass * sub = robj->Get_Sub_Object( i );
		if ( sub->Class_ID() == RenderObjClass::CLASSID_PARTICLEEMITTER && muzzle ) {
			sub->Set_Hidden( hidden );
		} else if ( sub->Class_ID() == RenderObjClass::CLASSID_HLOD ) {
			Set_Muzzle_Smoke_Emitters_Hidden( sub, muzzle, hidden );
		}
		sub->Release_Ref();
	}
}

static void Maybe_Trigger_Muzzle_Smoke( RenderObjClass * robj, bool under_muzzle )
{
	if ( robj == NULL ) {
		return;
	}
	...
	for ( int i = 0; i < robj->Get_Num_Sub_Objects(); i++ ) {
		...
		if ( sub->Class_ID() == RenderObjClass::CLASSID_PARTICLEEMITTER && muzzle ) {
			if ( FreeRandom.Get_Int( 2 ) == 0 ) {
				sub->Set_Hidden( false );
				((ParticleEmitterClass *)sub)->Start();
			}
		} else if ( sub->Class_ID() == RenderObjClass::CLASSID_HLOD ) {
			Maybe_Trigger_Muzzle_Smoke( sub, muzzle );
		}
		sub->Release_Ref();
	}
}
...
static float		MuzzleFlashHoldTimer;
...
void 	WeaponViewClass::Init()
{
	...
	LastMuzzleFlash = false;
	MuzzleFlashHoldTimer = 0.0f;
...
bool	WeaponViewClass::Load( ChunkLoadClass &cload )
{
	...
	// Saved hands carry a stale world transform; always recreate from the camera.
	if ( HandsPhysObj != NULL ) {
		DecorationPhysClass * stale_hands = HandsPhysObj;
		HandsPhysObj = NULL;
		if ( COMBAT_SCENE != NULL && COMBAT_SCENE->Contains( stale_hands ) ) {
			COMBAT_SCENE->Remove_Object( stale_hands );
		}
		stale_hands->Release_Ref();
	}
	WeaponViewEnabled = false;
...
/*
** Remove first-person hand decorations left in the dynamic physics scene by old saves.
*/
void WeaponViewClass::Purge_Orphan_Hands_From_Scene( void )
{
	...
	enum { MAX_ORPHAN_HANDS = 16 };
	PhysClass * orphans[MAX_ORPHAN_HANDS];
	int orphan_count = 0;

	RefPhysListIterator it = scene->Get_Dynamic_Object_Iterator();
	for ( it.First(); !it.Is_Done(); it.Next() ) {
		PhysClass * obj = it.Peek_Obj();
		if ( obj == NULL || obj->As_DecorationPhysClass() == NULL ) {
			continue;
		}
		if ( obj == HandsPhysObj ) {
			continue;
		...
		if ( strnicmp( name, "F_HM_", 5 ) != 0 && strstr( name, "F_HM_" ) == NULL ) {
			continue;
		}

		if ( orphan_count < MAX_ORPHAN_HANDS ) {
			orphans[orphan_count++] = obj;
		}
	}

	for ( int i = 0; i < orphan_count; i++ ) {
		PhysClass * obj = orphans[i];
...
void 	WeaponViewClass::Think()
{
	...
	if ( COMBAT_STAR ) {
		...
		if ( star_weapon != NULL ) {
			if ( star_weapon->Is_Reloading() ) {
				...
				// DidFire is one frame per shot; hold flash long enough to see at high FPS.
				MuzzleFlashHoldTimer = 0.08f;
	...
	MuzzleFlashHoldTimer -= TimeManager::Get_Frame_Seconds();
	if ( MuzzleFlashHoldTimer < 0.0f ) {
		MuzzleFlashHoldTimer = 0.0f;
	}
	muzzle_flash_on = MuzzleFlashHoldTimer > 0.0f;
...
//	if ( cur_weapon_model_name.Compare_No_Case( new_weapon_model_name ) != 0 ) {
	...
	if ( WeaponModel ) {
		// Flash meshes: sync every frame. Smoke emitters: random per shot, not every flash.
		RenderObjClass * model = WeaponModel;
		if ( model ) {
			for (int i=0; i<model->Get_Num_Sub_Objects(); i++) {
				RenderObjClass * robj = model->Get_Sub_Object(i);
				if ( Is_Muzzle_Fx_Name( robj->Get_Name() ) ) {
					Set_Muzzle_Fx_Mesh_Hidden( robj, !muzzle_flash_on );
				}
				robj->Release_Ref();
			}

			if ( fired_this_frame ) {
				Maybe_Trigger_Muzzle_Smoke( model, false );
			}
			if ( !muzzle_flash_on ) {
				Set_Muzzle_Smoke_Emitters_Hidden( model, false, true );
			}
...
//		Debug_Say(( "Loaded %s\n", WeaponModel ? WeaponModel->Get_Name() : "NONE" ));
		...
		// Use model name
		if ( WeaponModel != NULL ) {
			/* Copy model suffix (F_GM_RIFL -> RIFL); do not use Get_Name()+5 (non-owning pointer). */
			weapon_name = WeaponModel->Get_Name();
			if ( weapon_name.Get_Length() > 5 ) {
				weapon_name.Erase( 0, 5 );
			}
	...
	if ( WeaponModel != NULL ) {
		...
		for ( int i = 0; i < NUM_WEAPON_STATES; i++ ) {
			...
			anim_name.Format( "%s.F_GA_%s_%s", (const char *)weapon_htree_name, (const char *)weapon_name,
				WeaponActionNames[state] );
//			Debug_Say(( "Loading Weapon Anim %s\n", anim_name ));
			WeaponAnims[i] = WW3DAssetManager::Get_Instance()->Get_HAnim( (const char *)anim_name );
...
//			Debug_Say(( "Loading Hands Anim %s\n", anim_name ));
			HandsAnims[i] = WW3DAssetManager::Get_Instance()->Get_HAnim( (const char *)anim_name );
...
//	Debug_Say(( "Loading Hands from %s\n", name ));
	...
	if ( HandsPhysObj != NULL ) {
		...
		HandsPhysObj->Enable_Dont_Save( true );
...
//		Debug_Say(( "New Camera Bob State %d \"%s\"\n", BobState, name ));
		...
		if ( !name.Is_Empty() ) {
			...
			/*
			** Build anim/hierarchy names in stack buffers. Temporary StringClass slots are
			** reused by Ensure_W3D_File_Loaded / Get_HTree and can clobber a temp anim name
			** before Get_HAnim reads it (peek by literal succeeds, Get_HAnim by StringClass fails).
			*/
			char anim_name[ 2 * W3D_NAME_LEN + 2 ];
			const char *bob_name = name.Peek_Buffer();
			...
			asset_mgr->Ensure_W3D_File_Loaded( bob_name );
			asset_mgr->Clear_HAnim_Missing( anim_name );

			BobHTree = asset_mgr->Get_HTree( bob_name );
			BobHAnim = asset_mgr->Get_HAnim( anim_name );
			if ( BobHAnim == NULL ) {
				BobHAnim = asset_mgr->Resolve_HAnim( anim_name );
			}
			if ( BobHTree == NULL ) {
				BobHTree = asset_mgr->Get_HTree( bob_name );
			}
