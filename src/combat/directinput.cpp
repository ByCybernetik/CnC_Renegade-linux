...
static bool DIKeyboardPrevDown[ DirectInput::NUM_KEYBOARD_BUTTONS ];
static bool DIScancodeHeld[ DirectInput::NUM_KEYBOARD_BUTTONS ];

static int directinput_scancode_to_dik( UINT scancode, bool extended )
{
	if ( extended ) {
		switch ( scancode ) {
			case 0x1D: return DIK_RCONTROL;
			case 0x38: return DIK_RMENU;
			case 0x5B: return DIK_RWIN;
			case 0x5C: return DIK_APPS;
			case 0x52: return DIK_INSERT;
			case 0x53: return DIK_DELETE;
			case 0x47: return DIK_HOME;
			case 0x4F: return DIK_END;
			case 0x49: return DIK_PRIOR;
			case 0x51: return DIK_NEXT;
			case 0x48: return DIK_UP;
			case 0x4B: return DIK_LEFT;
			case 0x4D: return DIK_RIGHT;
			case 0x50: return DIK_DOWN;
			case 0x35: return DIK_NUMPADSLASH;
			case 0x1C: return DIK_NUMPADENTER;
			default: break;
		}
	}

	if ( scancode < DirectInput::NUM_KEYBOARD_BUTTONS ) {
		return (int)scancode;
	}
...
/*
** Wine/RU: WM_KEY lParam scancode can disagree with DIK (log: vk=87, sc=97, need DIK_W=17).
** Latin VK matches US key caps used in keynames.ini / Input mappings.
*/
static int directinput_latin_vk_to_dik( UINT vk )
{
	if ( vk >= 'a' && vk <= 'z' ) {
		vk = vk - 'a' + 'A';
	}
...
static int directinput_resolve_dik( WPARAM wParam, LPARAM lParam )
{
	UINT sc_lparam;
	UINT sc_vk;
	bool extended;
	...
	sc_lparam = ( (UINT)lParam >> 16 ) & 0xFF;
	extended = ( ( (UINT)lParam >> 24 ) & 1 ) != 0;
	sc_vk = 0;

	if ( wParam != 0 ) {
		sc_vk = MapVirtualKeyEx( (UINT)wParam, MAPVK_VK_TO_VSC, GetKeyboardLayout( 0 ) ) & 0xFF;
	}

	dik = directinput_latin_vk_to_dik( (UINT)wParam );
	if ( dik >= 0 ) {
		return dik;
	}

	if ( sc_lparam != 0 ) {
		dik = directinput_scancode_to_dik( sc_lparam, extended );
		if ( dik >= 0 ) {
			return dik;
		}
	}
	...
	if ( sc_vk != 0 ) {
		dik = directinput_scancode_to_dik( sc_vk, extended );
		if ( dik >= 0 ) {
			return dik;
		}
	}
...
#if defined(__MINGW32__) || defined(__MINGW64__)
	Win32_Key_Notify_Callback_Ptr = DirectInput::Notify_Win32_Key;
#endif
...
#if defined(__MINGW32__) || defined(__MINGW64__)
	if ( Win32_Key_Notify_Callback_Ptr == DirectInput::Notify_Win32_Key ) {
		Win32_Key_Notify_Callback_Ptr = NULL;
	}
#endif
}
...
#if defined(__MINGW32__) || defined(__MINGW64__)
	memset( DIKeyboardPrevDown, 0, sizeof( DIKeyboardPrevDown ) );
	memset( DIScancodeHeld, 0, sizeof( DIScancodeHeld ) );
#endif
}
...
void DirectInput::Notify_Win32_Key( UINT message, WPARAM wParam, LPARAM lParam )
{
	bool down;
	bool up;
	int dik;
	...
	dik = directinput_resolve_dik( wParam, lParam );
	if ( dik < 0 || dik >= NUM_KEYBOARD_BUTTONS ) {
		return;
	}

	if ( down ) {
		DIScancodeHeld[ dik ] = true;
		LastKeyPressed = dik;
	} else if ( up ) {
		DIScancodeHeld[ dik ] = false;
	}
}
...
#if defined(__MINGW32__) || defined(__MINGW64__)
	/*
	** Wine DirectInput ignores keyboard layout; WM_* scancodes do not.
	*/
	{
		int i;

		for ( i = 0; i < NUM_KEYBOARD_BUTTONS; i++ ) {
			bool down = DIScancodeHeld[ i ];
			int btn = 0;

			if ( down ) {
				btn |= DI_BUTTON_HELD;
			}
			if ( down && !DIKeyboardPrevDown[ i ] ) {
				btn |= DI_BUTTON_HIT;
			}
			if ( !down && DIKeyboardPrevDown[ i ] ) {
				btn |= DI_BUTTON_RELEASED;
			}

			DIKeyboardButtons[ i ] = (char)btn;
			DIKeyboardPrevDown[ i ] = down;
		}
