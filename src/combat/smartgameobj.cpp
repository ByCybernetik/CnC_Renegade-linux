...
bool	SmartGameObj::Save( ChunkSaveClass & csave )
{
	...
	csave.Begin_Chunk( CHUNKID_VARIABLES );
		...
		void * ptr = &Controller;
		WRITE_MICRO_CHUNK_WIRE_POINTER( csave, MICROCHUNKID_CONTROLLER_PTR, ptr );
		WRITE_MICRO_CHUNK( csave, MICROCHUNKID_MOVING_SOUND_TIMER, MovingSoundTimer ); 
		WRITE_MICRO_CHUNK_WIRE_POINTER( csave, MICROCHUNKID_PLAYER_DATA, PlayerData ); 
			...
			case CHUNKID_VARIABLES:
				...
				while (cload.Open_Micro_Chunk()) {
					switch(cload.Cur_Micro_Chunk_ID()) {
						...
						READ_MICRO_CHUNK_WIRE_POINTER( cload, MICROCHUNKID_PLAYER_DATA, PlayerData ); 
...
void SmartGameObj::On_Post_Load(void)
{
	...
	// Saved MoveablePhys may retain a stale Controller pointer after pointer remap;
	// always wire physics back to this object's embedded controller.
	PhysClass * phys = Peek_Physical_Object();
	if ( phys != NULL ) {
		MoveablePhysClass * moveable = phys->As_MoveablePhysClass();
		if ( moveable != NULL ) {
			moveable->Set_Controller( &Controller );
		}

		HumanPhysClass * human = phys->As_HumanPhysClass();
		if ( human != NULL && Is_Human_Controlled() ) {
			human->Force_Awake();
			human->Enable_User_Control( true );
		}
	}
