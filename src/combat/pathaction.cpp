...
PathActionClass::Save (ChunkSaveClass &csave)
{
	csave.Begin_Chunk (CHUNKID_VARIABLES);
		...
		WRITE_MICRO_CHUNK_WIRE_POINTER (csave, VARID_PATH_PTR,			Path);
		WRITE_MICRO_CHUNK_WIRE_POINTER (csave, VARID_MECHANISM_PTR,	Mechanism);
		WRITE_MICRO_CHUNK (csave, VARID_DESTINATION,		Destination);
		WRITE_MICRO_CHUNK (csave, VARID_FACEPOS,			FacePos);
		WRITE_MICRO_CHUNK_WIRE_POINTER (csave, VARID_GAME_OBJ_PTR,	GameObj);
...
PathActionClass::Load_Variables (ChunkLoadClass &cload)
{
	...
	while (cload.Open_Micro_Chunk ()) {
		switch (cload.Cur_Micro_Chunk_ID ()) {
			...
			READ_MICRO_CHUNK_WIRE_POINTER (cload, VARID_PATH_PTR,			Path);
			READ_MICRO_CHUNK_WIRE_POINTER (cload, VARID_MECHANISM_PTR,	Mechanism);
			READ_MICRO_CHUNK (cload, VARID_DESTINATION,		Destination);
			READ_MICRO_CHUNK (cload, VARID_FACEPOS,			FacePos);
			READ_MICRO_CHUNK_WIRE_POINTER (cload, VARID_GAME_OBJ_PTR,		GameObj);
