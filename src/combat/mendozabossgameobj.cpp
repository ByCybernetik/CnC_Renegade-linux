	...
	SideKickPos (0, 0, 0)
{
	...
	//	
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, OverallState, OVERALL_STATE_FLYING_SIDEKICK);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, OverallState, OVERALL_STATE_MELEE_ATTACK);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, OverallState, OVERALL_STATE_RANGED_ATTACK);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, OverallState, OVERALL_STATE_FIREBALL_ATTACK);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, OverallState, OVERALL_STATE_FIND_HEALTH);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, OverallState, OVERALL_STATE_SYDNEY_BOLTS);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, OverallState, OVERALL_STATE_RUN_AFTER_SYDNEY);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, OverallState, OVERALL_STATE_TOY_WITH_SYDNEY);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, OverallState, OVERALL_STATE_DEATH_SEQUENCE);
	
	//
	//	Register the Move states with its state machine
	//
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, MoveState, MOVE_STATE_STOP);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, MoveState, MOVE_STATE_GET_CLOSE_TO_PLAYER);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, MoveState, MOVE_STATE_ATTACK_PATTERN1);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, MoveState, MOVE_STATE_ATTACK_PATTERN2);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, MoveState, MOVE_STATE_GOTO_HELIPAD);	
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, MoveState, MOVE_STATE_RUN_TO_HEALTH);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, MoveState, MOVE_STATE_CHASE_SYDNEY);

	//
	//	Register the Mendoza states with its state machine
	//
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, MendozaState, MENDOZA_STATE_STANDING);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, MendozaState, MENDOZA_STATE_FALLING);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, MendozaState, MENDOZA_STATE_KIPPING);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, MendozaState, MENDOZA_STATE_PACK_EXPLODING);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, MendozaState, MENDOZA_STATE_DYING);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, MendozaState, MENDOZA_STATE_DEAD);
	MendozaState.Set_State (MENDOZA_STATE_STANDING);
	
	//
	//	Register the Sydney states with its state machine
	//
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, SydneyState, SYDNEY_STATE_WAITING);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, SydneyState, SYDNEY_STATE_BOLTING);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, SydneyState, SYDNEY_STATE_TRIPPING);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, SydneyState, SYDNEY_STATE_GETTING_UP);	
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, SydneyState, SYDNEY_STATE_COWERING);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, SydneyState, SYDNEY_STATE_STRIKE_A_POSE);	

	//
	//	Register the Head states with its state machine
	//
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, HeadState, HEAD_STATE_NONE);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, HeadState, HEAD_STATE_LOOKING_AT_STAR);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, HeadState, HEAD_STATE_LOOKING_AT_SYDNEY);

	//
	//	Register the attack states with its state machine
	//
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, AttackState, ATTACK_STATE_NONE);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, AttackState, ATTACK_STATE_MELEE);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, AttackState, ATTACK_STATE_MELEE_FLYING_SIDEKICK_RUN);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, AttackState, ATTACK_STATE_MELEE_FLYING_SIDEKICK);	
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, AttackState, ATTACK_STATE_MELEE_CRESENT_KICK);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, AttackState, ATTACK_STATE_MELEE_SIDE_KICK);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, AttackState, ATTACK_STATE_MELEE_SIDE_KICK_RETRACT);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, AttackState, ATTACK_STATE_MELEE_PUNCH);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, AttackState, ATTACK_STATE_FLAMETHROWER);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, AttackState, ATTACK_STATE_FIREBALL);	
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, AttackState, ATTACK_STATE_HANDGUN);
	ADD_STATE_TO_MACHINE_IMPL(MendozaBossGameObjClass, AttackState, ATTACK_STATE_SYDNEY);
	AttackState.Set_State (ATTACK_STATE_NONE);	

	//
	//	Register the camera states with its state machine
	//
	CameraState.Add_State (
		(StateClass<MendozaBossGameObjClass>::THINK_PTR)NULL,
		(StateClass<MendozaBossGameObjClass>::REQUEST_END_PTR)NULL,
		&MendozaBossGameObjClass::On_CAMERA_STATE_NORMAL_Begin,
		(StateClass<MendozaBossGameObjClass>::END_PTR)NULL);
	CameraState.Add_State (
		&MendozaBossGameObjClass::On_CAMERA_STATE_FACE_ZOOM_Think,
		(StateClass<MendozaBossGameObjClass>::REQUEST_END_PTR)NULL,
		&MendozaBossGameObjClass::On_CAMERA_STATE_FACE_ZOOM_Begin,
		(StateClass<MendozaBossGameObjClass>::END_PTR)NULL);
	CameraState.Add_State (
		&MendozaBossGameObjClass::On_CAMERA_STATE_WAYPATH_FOLLOW_Think,
		(StateClass<MendozaBossGameObjClass>::REQUEST_END_PTR)NULL,
		&MendozaBossGameObjClass::On_CAMERA_STATE_WAYPATH_FOLLOW_Begin,
		(StateClass<MendozaBossGameObjClass>::END_PTR)NULL);
	CameraState.Add_State (
		&MendozaBossGameObjClass::On_CAMERA_STATE_LOOK_AT_DEAD_BOSS_Think,
		(StateClass<MendozaBossGameObjClass>::REQUEST_END_PTR)NULL,
		&MendozaBossGameObjClass::On_CAMERA_STATE_LOOK_AT_DEAD_BOSS_Begin,
		(StateClass<MendozaBossGameObjClass>::END_PTR)NULL);	
