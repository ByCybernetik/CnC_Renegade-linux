...
StyleMgrClass::Initialize_From_INI (const char *filename)
{
	...
	if (file_obj != NULL) {
		ini_available = file_obj->Is_Available() ? 1 : 0;
		if (ini_available) {
...
)
{
	...
	if (do_vcenter) {
		...
		y_pos = int(rect.Top + (rect.Height () / 2.0F) - ((row_count * text_height) * 0.5F));
	}

	Vector2 text_extent = renderer->Get_Formatted_Text_Extents (text);
	if (justify == RIGHT_JUSTIFY) {
		x_pos = int(rect.Right - text_extent.X);
	} else if (justify == CENTER_JUSTIFY) {
		x_pos = int(rect.Left + (rect.Width () / 2) - (text_extent.X / 2));
	...
	//
	//	Build all wrapped lines in one pass so earlier draw quads stay valid.
	//
	renderer->Build_Sentence (text);
