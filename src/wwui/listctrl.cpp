...
static bool list_heap_ptr_ok(const void *ptr)
{
	if (ptr == NULL) {
		return false;
	}

	const unsigned long p = (unsigned long)ptr;
	return (p >= 0x00100000UL && p < 0x7F000000UL);
}
#endif
...
ListCtrlClass::Get_Next_Selected (int index) const
{
	...
	//
	int entry_count = Get_Entry_Count ();
	for (int idx = index + 1; idx < entry_count; idx ++) {
		
		//
		//	Is this entry selected?
		//
		if (RowInfoList[idx]->Is_Selected ()) {
			retval = idx;
			break;
		}
	}
...
{
#if defined(__GNUC__) && defined(_WIN32)
	if (!ColList.Has_Valid_Storage()) {
		ColList.Abandon_Corrupted_Storage();
		Set_Dirty();
		return;
	}
#endif
...
{
#if defined(__GNUC__) && defined(_WIN32)
	if (!EntryList.Has_Valid_Storage()) {
		EntryList.Abandon_Corrupted_Storage();
		return;
	}
#endif

	//
	//	Free each of the entries in the list (delete from end — safe if Count is wrong).
	//
	while (EntryList.Count () > 0) {
		const int index = EntryList.Count () - 1;
		ListEntryClass *entry = EntryList[index];

#if defined(__GNUC__) && defined(_WIN32)
		if (!list_heap_ptr_ok(entry)) {
			EntryList.Delete(index);
			continue;
		}
#endif
