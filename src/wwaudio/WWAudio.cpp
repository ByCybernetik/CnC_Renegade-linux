...
static bool memory_is_readable(const void *addr, size_t bytes)
{
#if defined(RENEGADE_LINUX)
	...
	uintptr_t page = start & ~((uintptr_t)page_size - 1U);
	...
	start = (const unsigned char *)addr;
	end = start + bytes;
	region_end = (const unsigned char *)mbi.BaseAddress + mbi.RegionSize;
	return (end <= region_end);
#endif
}


static bool pointer_is_plausible_object(const unsigned long p)
{
#if defined(RENEGADE_LINUX)
	const uintptr_t addr = (uintptr_t)p;
	/* amd64 canonical user-space; PIE heap/text live around 0x55…–0x7f… */
	if (addr < 0x10000UL || addr > 0x00007FFFFFFFFFFFUL) {
		return false;
	}
#else
	...
	if (p >= 0x00300000UL && p < 0x00400000UL) {
		return false;
	}
#endif

	if ((p & 0xFFFF0000UL) == 0xDDDD0000UL || (p & 0xFFFF0000UL) == 0xFEEE0000UL) {
		return false;
	}
...
static bool audible_sound_ptr_ok(const AudibleSoundClass *sound);
...
static bool audible_sound_ptr_ok(const AudibleSoundClass *sound)
{
	...
	const unsigned long p = (unsigned long)(uintptr_t)sound;
	if (!pointer_is_plausible_object(p)) {
		return false;
	}

	if (!memory_is_readable(sound, sizeof(void *))) {
		return false;
	}

	const unsigned long *vtable = *(const unsigned long *const *)sound;
	if (vtable == NULL) {
		return false;
	}

	const unsigned long vp = (unsigned long)(uintptr_t)vtable;
#if defined(RENEGADE_LINUX)
	if (!pointer_is_plausible_object(vp)) {
		return false;
	}
	if (!memory_is_readable(vtable, sizeof(unsigned long))) {
		return false;
	}
	return true;
#else
	if (vp < 0x00400000UL || vp >= 0x00B00000UL) {
		return false;
	}

	if (!memory_is_readable(vtable, 0xB8)) {
		return false;
	}

	unsigned long method = vtable[0xB4 / sizeof(unsigned long)];
	if (method < 0x00400000UL || method >= 0x00B00000UL) {
		return false;
	}
...
static bool scrub_playlist_vector(DynamicVectorClass<AudibleSoundClass *> &playlist)
{
	if (!playlist.Has_Valid_Storage()) {
		playlist.Abandon_Corrupted_Storage();
		return true;
	}
	...
	for (int index = 0; index < count; index++) {
		AudibleSoundClass *sound = playlist[index];
		if (sound != NULL && !audible_sound_ptr_ok(sound)) {
			playlist.Abandon_Corrupted_Storage();
			return true;
		}
	}
...
{
	if (owner == NULL || !audible_sound_ptr_ok (owner)) {
		return false;
	}
	...
	return AudibleSound_Is_Level_Ambient_Bed (owner);
}
...
{
	return sample != NULL && ::AIL_sample_loop_count (sample) == 0;
}
	...
	const AudibleSoundClass &requester)
{
	...
	return (owner == const_cast<AudibleSoundClass *>(&requester));
}
...
WWAudioClass::Begin_Reload_Sound_Playback (void)
{
	...
	** SDL3 voice_stream_is_playing tracks playback_end_ms reliably — skip wall-clock gate.
	*/
#if !defined(RENEGADE_LINUX)
...
WWAudioClass::Free_Completed_Sounds (void)
{
	...
	for (int index = 0; index < completed_count; index ++) {
		...
		if (!audible_sound_ptr_ok(sound_obj)) {
			m_CompletedSounds.Abandon_Corrupted_Storage();
			return;
		}
...
#endif
			} else if (audible_sound_ptr_ok (owner) &&
					!wwaudio_is_protected_2d_owner (owner)) {
