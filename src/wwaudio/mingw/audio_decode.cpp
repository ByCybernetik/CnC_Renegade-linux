/*
** WAV / IMA ADPCM / MP3 (mpg123) → PCM for OpenAL.
*/
...
/* Microsoft DVI ADPCM (WAVE format 0x11), stereo layout per dr_wav:
 * 8-byte block header (L predictor+step, R predictor+step), then 4 bytes L + 4 bytes R per 8 frames. */
...
int audio_decode_probe(const void *data, unsigned long data_len, AILSOUNDINFO *info)
{
	...
	if (mp3_buffer_looks_like_mp3(data, data_len)) {
		memset(info, 0, sizeof(*info));
		info->format = 0x0055; /* MPEG layer 3 tag used by Miles */
		info->rate = 44100;
		info->bits = 16;
		info->channels = 2;
		info->data_ptr = data;
	...
	AILSOUNDINFO *info)
{
	...
	*out_pcm_bytes = 0;
	...
	if (mp3_buffer_looks_like_mp3(data, data_len)) {
#if defined(WWAUDIO_HAVE_MPG123)
		return decode_mp3_mpg123(data, data_len, out_pcm, out_pcm_bytes, info);
#else
		return 0;
#endif
	}
...
int audio_load_file(const char *filename, unsigned char **out_data, unsigned long *out_len)
{
	...
	*out_data = (unsigned char *)malloc(size);
	...
	if (!ReadFile(file, *out_data, size, &read, NULL) || read != size) {
		free(*out_data);
		*out_data = NULL;
		CloseHandle(file);
		return 0;
