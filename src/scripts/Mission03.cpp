...
DECLARE_SCRIPT(M03_Commando_Script, "Controller_ID:int")
{
	...
	void Custom(GameObject * obj, int type, intptr_t param, GameObject * sender)
	{
		if (type == 3000)
		{
			int *occupied = (int *)(intptr_t)param;
			if (occupied != NULL)
			{
...
DECLARE_SCRIPT(M03_Chinook_Spawned_Soldier_GDI, "Area:int, Send_Type_When_Killed:int, Target_ID:int")
{
	...
	{
		int has_escort = 1;
		Commands->Send_Custom_Event(obj, poker, 3000, (intptr_t)&has_escort);
		if (has_escort == 1)
		{
...
DECLARE_SCRIPT(M03_Destroyed_Chinook, "Controller_ID:int, Simple_ID:int")
{
	void Killed(GameObject * obj, GameObject * killer)
	{
		/*int id = Commands->Create_Conversation("Chinook_Fodder_Scream", 100, 0, true);
		Commands->Join_Conversation(NULL, id, true, true, true);
		Commands->Join_Conversation(STAR, id, true, true, true);
		Commands->Start_Conversation(id, 0);*/
...
#define BEACH_FACING -180.0f
	...
	void Custom(GameObject * obj, int type, intptr_t param, GameObject * sender)
	{
		...
		if (type == 6300)
		{
			int *target_count = (int *)(intptr_t)param;
			if (beach_active)
			{
		...
		if (type == 6000 && param == 6000)
		{
			...
			{
				forced = true;
				/*int id = Commands->Create_Conversation("Nod_Reinforcements", 100, 0, true);
				Commands->Join_Conversation(NULL, id, true, true, true);
				Commands->Join_Conversation(Commands->Get_A_Star(Vector3(0,0,0)), id);
				Commands->Start_Conversation(id, 0);*/
...
DECLARE_SCRIPT(M03_Area_Troop_Counter, "")
{
	...
	void Custom(GameObject * obj, int type, intptr_t param, GameObject * sender)
	{
		if (type == 1000 && param == 1000)
		{
			int area = -1;
			int target_count = -1;
			Commands->Send_Custom_Event(obj, obj, 5000, (intptr_t)&area);
			Commands->Send_Custom_Event(obj, obj, 6300, (intptr_t)&target_count);
			if (area >= 0 && area <= 2)
			{
				area_count[area]--;
