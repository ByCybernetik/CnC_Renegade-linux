...
DECLARE_SCRIPT(MX0_Engineer_Goto, "GotoDest1:int, GotoDest2:int, Count:int")
{
	...
	void Custom(GameObject *obj, int type, intptr_t param, GameObject *sender)
	{
		...
		{
			Commands->Send_Custom_Event(obj, Commands->Find_Object(param), LEAD, Get_Int_Parameter("GotoDest1", 0), 0.0f );
		}

		if (type == RETURN_ENGINEER2)
		{
			Commands->Send_Custom_Event(obj, Commands->Find_Object(param), LEAD, Get_Int_Parameter("GotoDest2", 0), 0.0f );
		}
	}
	...
	void Entered (GameObject * obj, GameObject * enterer)
	{
		if (!already_entered)
		{
			...
			Commands->Send_Custom_Event(obj, Commands->Find_Object(1200001), COUNTER, Get_Int_Parameter("Count", 0), 0.0f );
...
DECLARE_SCRIPT(MX0_NOD_INFANTRY, "troop_num:int")
{
	...
	void Custom( GameObject *obj, int type, intptr_t param, GameObject *target )
	{
